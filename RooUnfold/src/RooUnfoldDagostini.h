...
inline RooUnfoldDagostini::RooUnfoldDagostini() : RooUnfold() {
  // Default constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfoldDagostini::RooUnfoldDagostini(const char* name, const char* title) : RooUnfold(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfoldDagostini::RooUnfoldDagostini(const TString& name, const TString& title) : RooUnfold(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfoldDagostini& RooUnfoldDagostini::operator=(const RooUnfoldDagostini& rhs) {
  // Assignment operator for copying RooUnfoldDagostini settings.
  Assign(rhs);
  return *this;
}

inline void RooUnfoldDagostini::SetIterations(Int_t niter) {
  // Set regularisation parameter (number of iterations)
  _niter = niter;
}

inline Int_t RooUnfoldDagostini::GetIterations() const {
  // Return regularisation parameter (number of iterations)
  return _niter;
}

inline void RooUnfoldDagostini::SetRegParm(Double_t parm) {
  // Set regularisation parameter (number of iterations)
  SetIterations(Int_t(parm + 0.5));
}

inline Double_t RooUnfoldDagostini::GetRegParm() const {
  // Return regularisation parameter (number of iterations)
