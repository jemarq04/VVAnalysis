...
def getManagerPath():
    path = "/cms/uhussain" if "hep.wisc.edu" in os.environ["HOSTNAME"] else "/afs/cern.ch/user/u/uhussain/work"
    return path
...
def getNumberAndSizeOfHDFSFiles(file_path):
    p = subprocess.Popen(
        ["hdfs", "dfs", "-ls", "-h", file_path.replace("/hdfs", "")],
...
def getListOfFiles(filelist, manager_path):
    ...
    for fullname in filelist:
        print("name in filelist: ", fullname)
        zz4l = "ZZ4l"
        Zl = "ZplusL"
        isTight = Zl not in fullname and "Tight" in fullname
        name = fullname.replace("Tight", "")
        if (zz4l in name) or (Zl in name):
            dataset_file = "%s/ZZ4lDatasetManager/FileInfo/%s/%s.json" % (
                manager_path,
                name,
                "LooseNtuples" if isTight else "ntuples",
            )
            print(dataset_file)
            if not os.path.isfile(dataset_file):
                raise ValueError(
                    "Invalid name in filelist: could not find '%s'. If desired, 'Tight' can also be in the name."
                    % dataset_file
                )
...
def getPreviousStep(selection, analysis):
    ...
    if "ZZ4l" in analysis:
        ...
        selection_map = {"ntuples": "ntuples", "ZplusLBase": "ntuples"}
...
def getTriggerName(sample_name, analysis, selection):
    ...
    if "Run" in sample_name and (
        getPreviousStep(selection, analysis) == "ntuples" or getPreviousStep(selection, analysis) == "LooseNtuples"
    ):
