...
public:
  ...
  double operator()(double* x, double* p) const {
    ...
    Int_t bin;
    if (_ndim == 1)
      bin = RooUnfoldResponse::FindBin(mes, x[0]);
    else if (_ndim == 2)
      bin = RooUnfoldResponse::FindBin(mes, x[0], x[1]);
    else
      bin = RooUnfoldResponse::FindBin(mes, x[0], x[1], x[2]);
    if (bin < 0 || bin >= _res->GetNbinsMeasured())
      return 0.0;
...
private:
  void FVals() const {
    ...
    for (Int_t i = 0, n = _res->GetNbinsTruth(); i < n; i++) {
      ...
      if (_eps <= 0.0) {
        if (_ndim >= 2)
          fv = _func->Eval(tru->GetXaxis()->GetBinCenter(jx), tru->GetYaxis()->GetBinCenter(jy), tru->GetZaxis()->GetBinCenter(jz));
        else
        ...
        if (_ndim == 1) {
          ...
          if (_ndim >= 3)
            fv /= tru->GetZaxis()->GetBinWidth(jz);
#else
          fv = _func->Integral(tlo, thi, (Double_t*)0, _eps) / tw;
        } else if (_ndim == 2) {
          fv = _func->Integral(tru->GetXaxis()->GetBinLowEdge(jx),
                               tru->GetXaxis()->GetBinUpEdge(jx),
                               tru->GetYaxis()->GetBinLowEdge(jy),
                               tru->GetYaxis()->GetBinUpEdge(jy),
                               _eps);
          fv /= tru->GetXaxis()->GetBinWidth(jx) * tru->GetYaxis()->GetBinWidth(jy);
        } else {
          fv = _func->Integral(tru->GetXaxis()->GetBinLowEdge(jx),
                               ...
                               tru->GetYaxis()->GetBinLowEdge(jy),
                               tru->GetYaxis()->GetBinUpEdge(jy),
                               tru->GetZaxis()->GetBinLowEdge(jz),
                               tru->GetZaxis()->GetBinUpEdge(jz),
                               _eps);
...
RooUnfoldResponse::RooUnfoldResponse(const RooUnfoldResponse& rhs) : TNamed(rhs.GetName(), rhs.GetTitle()) {
  //! RooUnfoldResponse copy constructor
  Init();
  Setup(rhs);
}

RooUnfoldResponse::RooUnfoldResponse(Int_t nb, Double_t xlo, Double_t xhi, const char* name, const char* title) : TNamed(name, title) {
  //! RooUnfoldResponse constructor - simple 1D case with same binning, measured vs truth
  Init();
  Setup(nb, xlo, xhi);
}

RooUnfoldResponse::RooUnfoldResponse(Int_t nm, Double_t mlo, Double_t mhi, Int_t nt, Double_t tlo, Double_t thi, const char* name, const char* title)
    : TNamed(name, title) {
...
RooUnfoldResponse::RooUnfoldResponse(const TH1* measured, const TH1* truth, const TH2* response, const char* name, const char* title) : TNamed(name, title) {
...
RooUnfoldResponse::RooUnfoldResponse(const TH1* measured, const TH1* truth, const char* name, const char* title) : TNamed(name, title) {
...
RooUnfoldResponse& RooUnfoldResponse::Init() {
  _overflow = 0;
  return Setup();
}

RooUnfoldResponse& RooUnfoldResponse::Setup() {
  _tru = _mes = _fak = 0;
  _res = 0;
...
RooUnfoldResponse& RooUnfoldResponse::Setup(const TH1* measured, const TH1* truth, const TH2* response) {
  ...
  if (_nm != _res->GetNbinsX() || _nt != _res->GetNbinsY()) {
    cerr << "Warning: RooUnfoldResponse measured X truth is " << _nm << " X " << _nt << ", but matrix is " << _res->GetNbinsX() << " X " << _res->GetNbinsY()
         << endl;
  }
...
Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t yr, Double_t xt, Double_t yt, Double_t w) {
  ...
  return _res->Fill(_res->GetXaxis()->GetBinCenter(FindBin(_mes, xr, yr) + 1), _res->GetYaxis()->GetBinCenter(FindBin(_tru, xt, yt) + 1), w);
}
...
Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t yr, Double_t zr, Double_t xt, Double_t yt, Double_t zt, Double_t w) {
  ...
  return _res->Fill(_res->GetXaxis()->GetBinCenter(FindBin(_mes, xr, yr, zr) + 1), _res->GetYaxis()->GetBinCenter(FindBin(_tru, xt, yt, zt) + 1), w);
}
...
TH1D* RooUnfoldResponse::H2H1D(const TH1* h, Int_t nb) {
...
TH2D* RooUnfoldResponse::HresponseNoOverflow() const {
...
TVectorD* RooUnfoldResponse::H2V(const TH1* h, Int_t nb, Bool_t overflow) {
...
TVectorD* RooUnfoldResponse::H2VE(const TH1* h, Int_t nb, Bool_t overflow) {
...
TMatrixD* RooUnfoldResponse::H2M(const TH2* h, Int_t nx, Int_t ny, const TH1* norm, Bool_t overflow) {
...
TMatrixD* RooUnfoldResponse::H2ME(const TH2* h, Int_t nx, Int_t ny, const TH1* norm, Bool_t overflow) {
...
void RooUnfoldResponse::Print(Option_t* /* option */) const { PrintMatrix(Mresponse(), Form("%s response matrix", GetTitle())); }

TH1* RooUnfoldResponse::ApplyToTruth(const TH1* truth, const char* name) const {
  ...
  TVectorD* resultvect;
  if (truth) {
    if (truth->GetNbinsX() != _tru->GetNbinsX() || truth->GetNbinsY() != _tru->GetNbinsY() || truth->GetNbinsZ() != _tru->GetNbinsZ())
      cerr << "Warning: RooUnfoldResponse::ApplyToTruth truth histogram is a different size (" << (truth->GetNbinsX() * truth->GetNbinsY() * truth->GetNbinsZ())
           << " bins) or shape from response matrix truth (" << (_tru->GetNbinsX() * _tru->GetNbinsY() * _tru->GetNbinsZ()) << " bins)" << endl;
...
TF1* RooUnfoldResponse::MakeFoldingFunction(TF1* func, Double_t eps, Bool_t verbose) const {
  ...
  if (TF3* func3 = dynamic_cast<TF3*>(func))
    f = new TF3(
        name, ROOT::Math::ParamFunctor(ff), func3->GetXmin(), func3->GetXmax(), func3->GetYmin(), func3->GetYmax(), func3->GetZmin(), func3->GetZmax(), np);
  else if (TF2* func2 = dynamic_cast<TF2*>(func))
    f = new TF2(name, ROOT::Math::ParamFunctor(ff), func2->GetXmin(), func2->GetXmax(), func2->GetYmin(), func2->GetYmax(), np);
  else
