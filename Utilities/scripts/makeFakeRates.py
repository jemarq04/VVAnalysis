...
def getComLineArgs():
    ...
    parser.add_argument(
        "--steps", type=str, choices=["merge", "ewk", "all"], default="all", help="step for fake rate calculation"
    )
...
def main():
    ...
    fileName = "fakeRate%s-%s.root" % (today, args["analysis"]) if args["output_file"] is None else args["output_file"]
    ...
    if args["steps"] in ["ewk", "all"]:
        ...
        alldata = HistTools.makeFakeRateCompositeHists(
            fOut, "AllData", ConfigureJobs.getListOfFilesWithXSec([args["analysis"] + "data"])
        )
        OutputTools.writeOutputListItem(alldata, fOut)
        alldata.Delete()

        allewk = HistTools.makeFakeRateCompositeHists(
            fOut,
            "AllEWK",
            ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfEWKFilenames("ZplusL%s" % args["year"])),
            True,
            lumi=args["lumi"],
        )
        ...
        allDYJets = HistTools.makeFakeRateCompositeHists(
            fOut,
            "DYMC",
            ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfDYFilenames("ZplusL%s" % args["year"])),
            True,
            lumi=args["lumi"],
        )
