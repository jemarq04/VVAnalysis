...
def addWeightsForState(original_file, state):
    ...
    for entry in range(tree.GetEntries()):
        ...
        pileup_branch.Fill()
        if state == "eem":
            mTightIso_weight[0] = ROOT.muonTightIsoSF(abs(tree.mEta), tree.mPt)
            mTightIso_branch.Fill()
        elif state == "emm":
            m1TightIso_weight[0] = ROOT.muonTightIsoSF(abs(tree.m1Eta), tree.m1Pt)
            m1TightIso_branch.Fill()
            m2TightIso_weight[0] = ROOT.muonTightIsoSF(abs(tree.m2Eta), tree.m2Pt)
            ...
            m2TightIso_weight[0] = ROOT.muonTightIsoSF(abs(tree.m2Eta), tree.m2Pt)
            m2TightIso_branch.Fill()
            m3TightIso_weight[0] = ROOT.muonTightIsoSF(abs(tree.m3Eta), tree.m3Pt)
