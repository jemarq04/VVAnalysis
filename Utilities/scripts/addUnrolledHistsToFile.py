...
transformed_mjj_etajj_hists = HistTools.getTransformedHists(
    input_file,
    ConfigureJobs.getListOfFiles(
    ...
    + ["mjj_etajj_2D_%s_%s" % (var, c) for c in ConfigureJobs.getChannels() for var in variations]
    + ["mjj_etajj_2D_%s_Fakes_%s" % (var, c) for c in ConfigureJobs.getChannels() for var in variations],
    HistTools.makeUnrolledHist,
    ConfigureJobs.get2DBinning(),
)
transformed_mjj_dRjj_hists = []
if adddRjj:
    transformed_mjj_dRjj_hists = HistTools.getTransformedHists(
        input_file,
        ConfigureJobs.getListOfFiles(
            ConfigureJobs.getListOfEWKFilenames()
            + [
        ...
        + ["mjj_dRjj_2D_%s_%s" % (var, c) for c in ConfigureJobs.getChannels() for var in variations]
        + ["mjj_dRjj_2D_%s_Fakes_%s" % (var, c) for c in ConfigureJobs.getChannels() for var in variations],
        HistTools.makeUnrolledHist,
        ConfigureJobs.get2DBinning("mjj", "dRjj"),
    )
