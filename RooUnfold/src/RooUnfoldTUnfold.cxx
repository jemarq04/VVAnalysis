...
RooUnfoldTUnfold::RooUnfoldTUnfold(const RooUnfoldTUnfold& rhs) : RooUnfold(rhs) {
  //! Copy constructor.
  Init();
  CopyData(rhs);
}

RooUnfoldTUnfold::RooUnfoldTUnfold(const RooUnfoldResponse* res, const TH1* meas, TUnfold::ERegMode reg, const char* name, const char* title)
    : RooUnfold(res, meas, name, title), _reg_method(reg) {
  //! Constructor with response matrix object and measured unfolding input histogram.
  Init();
}

RooUnfoldTUnfold::RooUnfoldTUnfold(const RooUnfoldResponse* res, const TH1* meas, Double_t tau, TUnfold::ERegMode reg, const char* name, const char* title)
    : RooUnfold(res, meas, name, title), _reg_method(reg) {
  //! Constructor with response matrix object and measured unfolding input histogram.
...
RooUnfoldTUnfold* RooUnfoldTUnfold::Clone(const char* newname) const {
  //! Clones object
...
void RooUnfoldTUnfold::Assign(const RooUnfoldTUnfold& rhs) {
  RooUnfold::Assign(rhs);
  CopyData(rhs);
}

void RooUnfoldTUnfold::CopyData(const RooUnfoldTUnfold& rhs) {
  tau_set = rhs.tau_set;
  _tau = rhs._tau;
...
void RooUnfoldTUnfold::Unfold() {
  //! Does the unfolding. Uses the optimal value of the unfolding parameter unless a value has already been set using FixTau

  if (_nm < _nt)
    cerr << "Warning: fewer measured bins than truth bins. TUnfold may not work correctly." << endl;
  if (_haveCovMes)
    cerr << "Warning: TUnfold does not account for bin-bin correlations on measured input" << endl;
  ...
  if (_res->FakeEntries()) {
    ...
    if (_verbose >= 1)
      cout << "Subtract " << fac * fakes.Sum() << " fakes from measured distribution" << endl;
    for (Int_t i = 1; i <= _nm; i++)
...
void RooUnfoldTUnfold::FixTau(Double_t tau) {
  //! Fix regularisation parameter to a specified value
  _tau = tau;
  tau_set = true;
}

void RooUnfoldTUnfold::SetRegMethod(TUnfold::ERegMode regmethod) {
  /*!
...
void RooUnfoldTUnfold::OptimiseTau() {
  //! Choose optimal regularisation parameter
  tau_set = false;
}

void RooUnfoldTUnfold::GetSettings() {
  _minparm = 0;
  _maxparm = 1;
  _stepsizeparm = 1e-2;
  _defaultparm = 2;
