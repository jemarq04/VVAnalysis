...
sys.path.append("/".join([manager_path, "ZZ4lRun2DatasetManager", "Utilities/python"]))
import ConfigHistTools

today = datetime.date.today().strftime("%d%b%Y")
tmpFileName = (
    "HistFiles/Hists%s-%s.root" % (today, args["leptonSelections"])
    if args["output_file"] == ""
...
hists = ConfigHistTools.getAllHistNames(manager_path, analysis) if "all" in args["hist_names"] else args["hist_names"]
...
if "FakeRate" not in args["output_selection"] and not args["test"]:
    ...
    background = SelectorTools.applySelector(
        ...
        + ConfigureJobs.getListOfDYFilenames()
        + ConfigureJobs.getListOfNonpromptFilenames()
        + ConfigureJobs.getListOfHZZFilenames(),
        channels,
        "ZZBackgroundSelector",
        args["selection"],
        fOut,
        args["analysis"],
        extra_inputs=sf_inputs + fr_inputs + hist_inputs + tselection,
