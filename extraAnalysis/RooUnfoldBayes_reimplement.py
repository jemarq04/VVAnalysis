...
class RooUnfoldResponse:
    ...
    def SetupMatrix(self, measured, truth, response):
        ...
        if self._nm != self._res.GetNbinsX() or self._nt != self._res.GetNbinsY():
            print(
                "Warning: RooUnfoldResponse measured X truth is ",
                self._nm,
                " X ",
                self._nt,
                ", but matrix is ",
        ...
        if not measured or self._mes.GetEntries() == 0.0:
            ...
            for i in range(nm):
                ...
                self._fak.SetBinError(
                    bin, math.sqrt(wmes + (pow(self._mes.GetBinError(bin), 2) if sm else self._mes.GetBinContent(bin)))
                )
...
class RooUnfold:
    ...
    def Init(self):
        ...
        self._verbose = 1
        self._overflow = 0
        self._dosys = self._unfolded = self._haveCov = self._haveCovMes = self._fail = self._have_err_mat = (
            self._haveErrors
        ) = self._haveWgt = False
    ...
    def UnfoldWithErrors(self, withError, getWeights=False):
        ...
        if not self._unfolded:
            ...
            if (
                self._meas.GetDimension() != rmeas.GetDimension()
                or self._meas.GetNbinsX() != rmeas.GetNbinsX()
...
class RooUnfoldBayes(RooUnfold):
    ...
    def setup(self):
        ...
        RooUnfoldBayes.H2M(
            self._res.Hresponse().Clone(), self._Nji, self._overflow
