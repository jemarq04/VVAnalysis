...
def getComLineArgs():
    ...
    parser.add_argument(
        "--scaleFacs",
        action="store_true",
        help="Add lepton and pilup scale factors to ntuple (by default they are not added)",
    )
    parser.add_argument(
        "--deduplicateAcrossChannels",
        action="store_true",
    ...
    parser.add_argument("-e", "--extraArgs", type=str, default="", help="Extra arguments to pass to skimNtuples script")
    parser.add_argument("--noSubmit", action="store_true", help="Create submit scripts but don't call farmout")
    parser.add_argument("--jobName", type=str, help="Optional name of job (defaults to auto-generation)")
...
def getFilesPerJob(path_to_files):
    ...
    if file_type == "Local":
        ...
        return int(math.ceil(200.0 / average_size))
...
def farmoutNtupleSkim(
    ...
    farmout_dict["input_files_path"] = ConfigureJobs.getInputFilesPath(
        sample_name, path, ConfigureJobs.getPreviousStep(selection, analysis), analysis
    )
    ...
    ConfigureJobs.fillTemplatedFile(
        "/".join([farmout_dict["base_dir"], "Templates/farmout_template.sh"]), script_name, farmout_dict
    )
    createRunJob(
...
def createRunJob(base_dir, job_dir, selection, analysis, trigger_name, addScaleFacs, addDuplicatedFlag, extraArgs):
    ...
    ConfigureJobs.fillTemplatedFile(
        "/".join([base_dir, "Templates/skim_template.sh"]), "/".join([job_dir, "skim.sh"]), fill_dict
    )
    ...
    for file_name in filelist:
        try:
            farmoutNtupleSkim(
                ...
                args["version"],
                args["scaleFacs"],
                args["deduplicateAcrossChannels"],
                args["noSubmit"],
                args["extraArgs"],
                args["jobName"],
            )
