...
RooUnfoldBayes::RooUnfoldBayes(const RooUnfoldBayes& rhs) : RooUnfold(rhs) {
  //! Copy constructor.
  Init();
  CopyData(rhs);
}

RooUnfoldBayes::RooUnfoldBayes(const RooUnfoldResponse* res, const TH1* meas, Int_t niter, Bool_t smoothit, const char* name, const char* title)
  ...
  for (Int_t kiter = 0; kiter < _niter; kiter++) {
    ...
    if (_dosys) {
      ...
      if (kiter > 0) {
        ...
        for (Int_t i = 0; i < _nc; i++) {
          if (_P0C[i] <= 0.0)
            continue;  // skip loop: dnCidPjkUpd(i,jk) will also be 0 because _Mij(i,j) will be 0
...
void RooUnfoldBayes::getCovariance() {
  if (_dosys != 2) {
    if (verbose() >= 1)
      cout << "Calculating covariances due to number of measured events" << endl;
...
//-------------------------------------------------------------------------
Double_t RooUnfoldBayes::getChi2(const TVectorD& prob1, const TVectorD& prob2, Double_t nevents) const {
...
void RooUnfoldBayes::Print(Option_t* option) const {
  ...
  if (((_nEstj.Sum()) != 0) || ((_nCi.Sum()) != 0)) {
    ...
    for (Int_t i = 0; i < iend; i++) {
      if ((_nCi[i] == 0) && (_nEstj[i] == 0) && (_nEstj[i] == 0) && (_nbarCi[i] == 0))
        continue;
      cout << i << "\t" << _nCi[i] << "\t " << _nEstj[i] << "\t " << _nbarCi[i] << endl;
    }
    ...
    cout << "--------------------------------------------------------" << endl;
    cout << " \t" << (_nCi.Sum()) << "\t " << (_nEstj.Sum()) << "\t " << (_nbarCi.Sum()) << endl;
    cout << "--------------------------------------------------------\n" << endl;
  }
}
