...
with open("controlevent.txt", "w") as outfile:
    ...
    file_info = json.load(
        open(
            "/afs/cern.ch/user/k/kelong/work/AnalysisDatasetManager/FileInfo/WZxsec2016/WselectionMediumLeptonsNoEIP.json"
        )
    )
    for i, chan in enumerate(["mmm", "eem", "emm", "eee"]):
        ...
        for row in cut_chain:
            outfile.write(
                "DATA "
                + " ".join([str(v) for v in [row.run, row.lumi, row.evt]])
                + " "
                + " ".join(
                    [
                        str(round(v, 2))
            ...
            if ptsort:
                ...
                if chan == "eee":
                    leps = list(
                        reversed(
                            sorted(
                                [(row.e1Pt, row.e1Eta), (row.e2Pt, row.e2Eta), (row.e3Pt, row.e3Eta)],
                                key=lambda x: x[0],
                            )
                        )
                    )
                    ...
                    leps = list(
                        reversed(
                            sorted(
                                [(row.e1Pt, row.e1Eta), (row.e2Pt, row.e2Eta), (row.mPt, row.mEta)], key=lambda x: x[0]
                            )
                        )
                    )
                elif chan == "emm":
                    leps = list(
                        reversed(
                            sorted(
                                [(row.m1Pt, row.m1Eta), (row.m2Pt, row.m2Eta), (row.ePt, row.eEta)], key=lambda x: x[0]
                            )
                        )
                    )
                if chan == "mmm":
                    leps = list(
                        reversed(
                            sorted(
                                [(row.m1Pt, row.m1Eta), (row.m2Pt, row.m2Eta), (row.m3Pt, row.m3Eta)],
                                key=lambda x: x[0],
                            )
                        )
                    )
                outfile.write(
                    " "
                    + " ".join([str(round(v, 4)) for v in [leps[0][0], leps[0][1], leps[1][0], leps[1][1]]])
                    + " %i\n" % i
                )
            else:
                if chan == "eee" or chan == "eem":
                    outfile.write(
                        " "
                        + " ".join([str(round(v, 2)) for v in [row.e1Pt, row.e1Eta, row.e2Pt, row.e2Eta]])
                        + " %i\n" % i
                    )
                elif chan == "emm" or chan == "mmm":
                    outfile.write(
                        " "
                        + " ".join([str(round(v, 2)) for v in [row.m1Pt, row.m1Eta, row.m2Pt, row.m2Eta]])
                        + " %i\n" % i
                    )
