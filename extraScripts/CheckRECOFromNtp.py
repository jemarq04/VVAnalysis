...
def percentDiff(h1, h2):
    list = [
        abs(h1.GetBinContent(i) - h2.GetBinContent(i)) / (h1.GetBinContent(i) + h2.GetBinContent(i)) * 2.0 * 100.0
...
xsecsUL = [1.325 * 1.0835, 1.6175 / 1000.0 * 1.7, 3.29188 / 1000.0 * 1.7, 1.59718 / 1000.0 * 1.7]
checkgen = False
if checkgen:
    xsecsUL = xsecspre
...
for var in vars:
            ...
            for _, VFP in lumipairs:
                ...
                for ds in datasets:
                    ...
                    for evt in tree:
                        ...
                        if "abs" in varp:
                            exec(
                                "h.Fill(abs(evt.%s),evt.%s*xsecs[j]*lumi*1000/sumweights%s)"
                                % (varp.replace("abs", ""), weightExpr, additional)
                            )
                            ...
                            if tmp_nj == 4:
                                if evt.nJets >= 4:
                                    exec(
                                        "h.Fill(evt.Mass,evt.%s*xsecs[j]*lumi*1000/sumweights%s)"
                                        % (weightExpr, additional)
                                    )
                            else:
                                if evt.nJets == tmp_nj:
                                    exec(
                            ...
                            exec(
                                "h.Fill(evt.%s,evt.%s*xsecs[j]*lumi*1000/sumweights%s)" % (varp, weightExpr, additional)
                            )
    ...
    for chan in channels + ["total"]:
        ...
        print(perlist)
        for perc in perlist:
            if chan == "total" and perc >= 3.0:
                print("WARNING: >3 percent for one entry in total")

            if chan == "mmmm" and perc >= 5.0:
