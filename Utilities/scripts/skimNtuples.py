...
def getComLineArgs():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        choices=["DoubleEG", "DoubleMuon", "MuonEG", "SingleMuon", "SingleElectron", "MonteCarlo", ""],
...
def skimNtuple(selections, analysis, trigger, filelist, output_file_name, deduplicate):
    ...
    with open(filelist) as input_file:
        input_files = [
            ("root://cmsxrootd.hep.wisc.edu/" + i.strip()) if "store" in i[:6] else i.strip() for i in input_file
        ]
