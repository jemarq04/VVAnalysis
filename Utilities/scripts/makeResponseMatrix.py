...
_binning = {
    ...
    "jetPt0": [30.0, 70.0, 110.0, 150.0, 190.0, 230.0, 270.0, 310.0, 350.0, 390.0, 430.0],
...
responseClassNames = {
    ...
    "jetPt0": dict.fromkeys(channels, "testJet"),
...
varNamesForResponseMaker = {
    ...
    "jetPt0": dict.fromkeys(channels, "jetPt0"),
...
def unfold(
    ...
    if not args["noSyst"]:
        ...
        for sys, scale in lumiScale.items():
            ...
            hUnfolded["lumi_" + sys], hCovLumi, hRespLumi = getUnfolded(
                hSigLumi, hBkgTotalLumi, hTrueLumiShift, hResponseLumi, hData, nIter, True
            )
        ...
        for lep in set(chan):
            ...
            for sys in ["Up", "Down"]:
                ...
                hUnfolded[lep + "Eff_" + sys], hCovLep, hRespLep = getUnfolded(
                    hSigSyst, hBkgSystTotal, hTruth[""], hRespSyst, hData, nIter, True
                )
                del hSigSyst
                del hBkgMCSyst
                del hBkgSyst
    ...
    hResponseAltNominal = dict(altResponseMakers)
    print("hResponseNominal:", hResponseNominal)
    print("hResponseAltNominal:", hResponseAltNominal)

    _hResponseSig7 = hResponseAltNominal["zz4l-amcatnlo"].getResponse("pu_Up")
...
def generatePlots(hUnfolded, hUncUp, hUncDn, hTruth, hTruthAlt, varName, norm, normFb, lumi, unfoldDir=""):
    ...
    if unfoldDir:
        ...
        if not norm and normFb:
            print(f"Inclusive fiducial cross section = {hUnf.Integral(0, hUnf.GetNbinsX() + 1)} fb")
        if norm or normFb:
            normalizeBins(hUnf)
...
def addTheoryVar(processName, varName, entries, central=0, exclude=None):
    if exclude is None:
        exclude = []
    theoryVariations = {}
    ...
    if processName not in theoryVariations:
        theoryVariations[processName] = {}

    theoryVariations[processName].update(
        {
            name: {
                "entries": entries,
                "central": central,
...
if args["test"]:
    sigSamplesPath = {}
    fUse = ROOT.TFile(
        "SystGenFiles/originalWithGenandSyst_forJets_1jetCutBoth_Hists12Oct2020-ZZ4l2016_Moriond.root", "update"
    )
...
for chan in channels:
    for syst in ["Up", "Down"]:
        for s in runVariables:
            systList.append(varNames[s] + "_CMS_pileup" + syst)
            for lep in set(chan):
                systList.append(varNames[s] + "_CMS_eff_" + lep + syst)
...
for varName in runVariables:
    ...
    for chan in channels:
        ...
        # gdb_debugger.hookDebugger()
        # pdb.set_trace()
        hUnfolded[chan], hTrue[chan], hTrueAlt[chan] = unfold(
            varName,
            chan,
            responseMakers,
            altResponseMakers,
    ...
    if args["makeTotals"]:
        ...
        hErrTot = _combineChannelUncertainties(*list(hErr.values()))
        hTotUncUp, hTotUncDn = _sumUncertainties(hErrTot, varName)
        generatePlots(
            hTot, hTotUncUp, hTotUncDn, hTrueTot, hTrueAltTot, varName, norm, normFb, args["lumi"], UnfoldOutDir
        )
