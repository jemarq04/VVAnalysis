...
def flagduplicates(IntersectionList, dictList):
    ...
    if len(eeee_eemm) > 0:
        ...
        for i in eeee_eemm:
            ...
            if (event_score_eeee[i][0] < event_score_eemm[i][0]) or (
                (event_score_eeee[i][0] == event_score_eemm[i][0]) and (event_score_eeee[i][1] > event_score_eemm[i][1])
            ):
                eemm_flag.append(i)
            else:
                eeee_flag.append(i)
    if len(eemm_mmmm) > 0:
        ...
        # mass_discriminant = score[0], Z2PtSum = score[1]
        for i in eemm_mmmm:
            if (event_score_eemm[i][0] < event_score_mmmm[i][0]) or (
                (event_score_eemm[i][0] == event_score_mmmm[i][0]) and (event_score_eemm[i][1] > event_score_mmmm[i][1])
            ):
                mmmm_flag.append(i)
            else:
                eemm_flag.append(i)
