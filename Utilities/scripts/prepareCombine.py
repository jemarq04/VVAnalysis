...
def getComLineArgs():
    ...
    parser.add_argument("--allWZSignal", action="store_true", help="Treat all WZjj as signal")
    parser.add_argument("--noTheory", action="store_true", help="Don't add theory hists")
    parser.add_argument("--addInterference", action="store_true", help="Add shape-based interference uncertainty")
    parser.add_argument("--nonpromptCombinedShapes", action="store_true", help="Use combined shape for nonprompt")
    parser.add_argument("--noCards", action="store_true", help="Don't create cards for combine")
    parser.add_argument("--aqgc", action="store_true", help="Add aqgc files")
    parser.add_argument("--higgs", action="store_true", help="Add higgs files")
    parser.add_argument("--manualStats", action="store_true", help="Stat uncertainties inserted by bin")
    ...
    parser.add_argument("--fit_variable", type=str, default="", help="Variable to use in the shape based fit")
    parser.add_argument("--combineChannels", action="store_true", help="Don't fit channels independently")
    parser.add_argument("--addControlRegion", action="store_true", help="Add control region to fit distribution")
    parser.add_argument(
        "-b",
        "--hist_names",
...
nonprompt = HistTools.makeCompositeHists(
    fIn,
    "DataEWKCorrected",
    {"DataEWKCorrected": 1},
    args["lumi"],
...
for var in jeVariations:
    hists = HistTools.makeCompositeHists(
        fIn,
        "DataEWKCorrected",
        {"DataEWKCorrected": 1},
        args["lumi"],
...
if args["aqgc"]:
    ...
    for filename in [
        "WZxsec2016_aQGC-FM.json",
        "WZxsec2016_aQGC-FS.json",
...
for plot_group in plot_groups:
    ...
    if scaleWZ and plot_group in wz_scalefacs:
        for h in group:
    ...
    for chan in chans:
        ...
        if "data" not in plot_group and not isNPpoint and not args["noTheory"]:
            ...
            if "TH2" in weight_hist.ClassName():
                ...
                scale_hists = HistTools.getTransformed3DScaleHists(
                    weight_hist,
                ...
                if pdf_entries[plot_group]:
                    pdf_hists = HistTools.getTransformed3DPDFHists(
                        weight_hist,
                        HistTools.makeUnrolledHist,
                        ConfigureJobs.get2DBinning(yvar="etajj" if "dRjj" not in variable else "dRjj"),
                ...
                raise RuntimeError(
                    "Invalid weight hist %s" % weight_hist_name + " for %s. Can't make scale variations" % plot_group
                )
...
if not args["noCards"]:
    ...
    for chan, chan_dict in card_info.iteritems():
        ...
        file_name = (
            "%s/WZjj%s_%s.txt" % (output_dir, signal_abv, chan) if isVBS else "%s/WZ_%s.txt" % (output_dir, chan)
        )
        ...
        ConfigureJobs.fillTemplatedFile(template_name, file_name, chan_dict)
