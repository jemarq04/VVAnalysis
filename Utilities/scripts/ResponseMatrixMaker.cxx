...
template <typename T>
ResponseMatrixMakerBase<T>::ResponseMatrixMakerBase(const Str& channel, const Str& varName, const Vec<float>& binning)
    : varName(varName),
      channel(channel),
      binning(binning),
      nPDFVariations(100),
...
void ResponseMatrixMakerBase<T>::setup() {
  ...
  UPtr<TChain> recoTree(new TChain((getChannel() + "/ntuple").c_str(), ("recoChain_" + getVar() + "_" + getChannel()).c_str()));
  for (const auto& fn : fileNames) {  //std::cout<<fn<<std::endl;
  ...
  Vec<Str> systs = Vec<Str>({
      "nominal", "pu_Up", "pu_Down", "jes_up", "jes_dn", "jer_up", "jer_dn",
      //"pdf_Up","pdf_Down",
  });

  Vec<Str> jsysts = Vec<Str>({
      "jes_up",
      "jes_dn",
      "jer_up",
      "jer_dn",
  ...
  if (hasLHE && !skipSyst) {                  //Change the codes to store all scale,PDF+alpha_s variations in a single vector.
    for (size_t i = 0; i <= lastLHEind; ++i)  //scale 0 to 8, followed by 103 pdf+alpha_s variations
    {
      scaleResponses.push_back(
          TH2D(("scalePDFAlphasVariation" + std::to_string(i)).c_str(), "", binning.size() - 1, &binning[0], binning.size() - 1, &binning[0]));
    ...
    if (!skipSyst) {
      ...
      pdfResponses = TH3D("pdfResponses", "", binning.size() - 1, &binning[0], binning.size() - 1, &binning[0], iterationBins.size() - 1, &iterationBins[0]);
  ...
  for (auto& s : systs) {
    responses[s] = TH2D(s.c_str(), "", binning.size() - 1, &binning[0], binning.size() - 1, &binning[0]);
  }
  ...
  // Get gen info
  UPtr<TChain> trueTree = UPtr<TChain>(new TChain((getChannel() + "Gen/ntuple").c_str(), ("trueChain_" + getVar() + "_" + getChannel()).c_str()));
  for (const auto& fn : fileNames)
    trueTree->Add(fn.c_str());

  UPtr<UMap<size_t, T> > trueVals = this->getTrueValues(*trueTree.get(), objects);
      ...
      if (!skipSyst) {
        ...
        if (hasLHE && pdfAndAlphaSWeights.at(0)) {
          ...
          for (size_t ind = 0; ind < 9; ++ind)
            this->fillResponse(scaleResponses.at(ind), val, trueVal, nominalWeightScaleNorm * scaleWeights.at(ind));
          /* float nominalWeightScaleNorm = nominalWeight / scaleWeights.at(0);
          ...
          for (size_t ind = 0; ind <= lastLHEind - 9; ++ind)
            this->fillResponse(scaleResponses.at(ind + 9), val, trueVal, nominalWeightScaleNorm * pdfAndAlphaSWeights.at(ind));
  ...
  for (auto& treeInfo : systTreesNeeded) {
    ...
    UPtr<TChain> t = UPtr<TChain>(new TChain((getChannel() + "/ntuple").c_str(), ("chain_" + getVar() + "_" + getChannel()).c_str()));
    ...
    for (size_t row = 0; row < size_t(std::abs(t->GetEntries())); ++row) {
      ...
      if (this->selectEvent(systName)) {
        this->fillResponse(responses[systName], this->getEventResponse(), trueVal, scale * puWt * lepSF * genWeight);
      }
    }
...
template <typename T>
float* ResponseMatrixMakerBase<T>::getmZPtr(TChain& t, const Str& obj1, const Str& obj2, float& maybeUseThis) const {
...
template <typename T>
float ResponseMatrixMakerBase<T>::getLepSF(const Vec<Str>& objects, float eSyst, float mSyst) {
  float out = 1.;
...
template <typename T>
BranchValueResponseMatrixMaker<T>::BranchValueResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SimpleValueResponseMatrixMakerBase<T>(channel, varName, binning) {
  ;
}

template <typename T>
UPtr<UMap<size_t, T> > BranchValueResponseMatrixMaker<T>::getTrueValues(TChain& t, const Vec<Str>& objects, const Str& syst) const {
  ...
  float _doNotUse1 = 0.;
  float _doNotUse2 = 0.;
  float* const mZ1 = this->getmZPtr(t, objects.at(0), objects.at(1), _doNotUse1);
  float* const mZ2 = this->getmZPtr(t, objects.at(2), objects.at(3), _doNotUse2);
...
template <typename T>
void BranchValueResponseMatrixMaker<T>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  if (this->getVar().find("Mass") != Str::npos) {
    t.SetBranchAddress("Mass", &value);
  }
  //std::cout<<"SetRecoBranches called========================================================"<<std::endl;}
  else if (this->getVar().find("nJets") == Str::npos && this->getVar().find("mjj") == Str::npos) {
    t.SetBranchAddress(this->getVar().c_str(), &value);
  }  //nJets and mjj already set elsewhere
}

template <typename T>
T BranchValueResponseMatrixMaker<T>::getEventResponse(const Str& opt) const {
  return value;
}

template <class R>
AbsValueResponseMatrixMaker<R>::AbsValueResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning) : R(channel, varName, binning) {
  ;
}

template <class R>
UPtr<UMap<size_t, typename R::ValType> > AbsValueResponseMatrixMaker<R>::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, typename R::ValType> > out = R::getTrueValues(trueTree, objects, syst);
  for (auto& val : *out)
...
template <class R>
typename R::ValType AbsValueResponseMatrixMaker<R>::getEventResponse(const Str& option) const {
...
template <class R>
typename R::ValType& AbsValueResponseMatrixMaker<R>::doAbs(typename R::ValType& val) const {
  val = std::abs(val);
  return val;
}

template <typename T>
JetBranchResponseMatrixMakerBase<T>::JetBranchResponseMatrixMakerBase(const Str& channel, const Str& varName, const Vec<float>& binning)
...
template <typename T>
void JetBranchResponseMatrixMakerBase<T>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  my_doNotUse1 = 0.;
  my_doNotUse2 = 0.;
...
template <class T>
DijetBranchResponseMatrixMaker<T>::DijetBranchResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : JetBranchResponseMatrixMakerBase<T>(channel, varName, binning) {
  ;
}

template <class T>
UPtr<UMap<size_t, T> > DijetBranchResponseMatrixMaker<T>::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, T> > out(new UMap<size_t, T>());
  ...
  if (this->getVar().find("jetPt") != Str::npos) {
    ...
    trueTree.SetBranchAddress("jetEta", &trueValvecfloat);
  } else if (this->getVar().find("Mass") != Str::npos) {
    trueTree.SetBranchAddress("Mass", &trueVal);
  }  //All Massnj variables use the Mass address, with nJets selection applied later below
  ...
  float _doNotUse1 = 0.;
  float _doNotUse2 = 0.;
  float* const mZ1 = this->getmZPtr(trueTree, objects.at(0), objects.at(1), _doNotUse1);
  float* const mZ2 = this->getmZPtr(trueTree, objects.at(2), objects.at(3), _doNotUse2);
  ...
  for (size_t row = 0; row < size_t(std::abs(trueTree.GetEntries())); ++row) {
    ...
    if (this->getVar() == "Mass") {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass0j" && trueNJets == 0) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass1j" && trueNJets == 1) {
      (*out)[trueEvt] = trueVal;
    }
    //std::cout<<trueEvt<<" "<<trueVal<<std::endl; }
    else if (this->getVar() == "Mass2j" && trueNJets == 2) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass3j" && trueNJets == 3) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass34j" && trueNJets >= 3) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass4j" && trueNJets >= 4) {
      (*out)[trueEvt] = trueVal;
    }

    if (this->getVar() == "MassFull") {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass0jFull" && trueNJets == 0) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass1jFull" && trueNJets == 1) {
      (*out)[trueEvt] = trueVal;
    }
    //std::cout<<trueEvt<<" "<<trueVal<<std::endl; }
    else if (this->getVar() == "Mass2jFull" && trueNJets == 2) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass3jFull" && trueNJets == 3) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass34jFull" && trueNJets >= 3) {
      (*out)[trueEvt] = trueVal;
    } else if (this->getVar() == "Mass4jFull" && trueNJets >= 4) {
      (*out)[trueEvt] = trueVal;
    }

    else if (this->getVar() == "nJets") {
      (*out)[trueEvt] = trueNJets;
    } else if (this->getVar() == "mjj" && trueNJets >= 2) {
      (*out)[trueEvt] = trueMjj;
    } else if (this->getVar() == "dEtajj" && trueNJets >= 2) {
      (*out)[trueEvt] = std::abs(trueValvecfloat->at(0) - trueValvecfloat->at(1));
    } else if (trueNJets >= 1 && this->getVar() == "jetPt[0]") {
      (*out)[trueEvt] = trueValvecfloat->at(0);
    } else if (trueNJets >= 2 && this->getVar() == "jetPt[1]") {
      (*out)[trueEvt] = trueValvecfloat->at(1);
    } else if (trueNJets >= 1 && this->getVar() == "jetEta[0]") {
      (*out)[trueEvt] = trueValvecfloat->at(0);
    } else if (trueNJets >= 2 && this->getVar() == "jetEta[1]") {
      (*out)[trueEvt] = trueValvecfloat->at(1);
    } else if (trueNJets >= 1 && this->getVar() == "absjetEta[0]") {
      (*out)[trueEvt] = std::abs(trueValvecfloat->at(0));
    } else if (trueNJets >= 2 && this->getVar() == "absjetEta[1]") {
      (*out)[trueEvt] = std::abs(trueValvecfloat->at(1));
    }
...
template <class T>
void DijetBranchResponseMatrixMaker<T>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
...
T DijetBranchResponseMatrixMaker<T>::getEventResponse(const Str& syst) const {
  ...
  // return mjj;}
    ...
    return JetBranchResponseMatrixMakerBase<T>::getEventResponse(syst);
  }
  //return BranchValueResponseMatrixMaker<T>::getEventResponse(syst);
}
...
template <class T>
bool DijetBranchResponseMatrixMaker<T>::selectEvent(const Str& syst) const {
  ...
  if (this->getVar() == "Mass0j") {
    return tmp_nJets == 0 && mass_sel;
  }
  if (this->getVar() == "Mass1j") {
    return tmp_nJets == 1 && mass_sel;
  }
  if (this->getVar() == "Mass2j") {
    return tmp_nJets == 2 && mass_sel;
  }
  if (this->getVar() == "Mass3j") {
    return tmp_nJets == 3 && mass_sel;
  }
  if (this->getVar() == "Mass34j") {
    return tmp_nJets >= 3 && mass_sel;
  }
  if (this->getVar() == "Mass4j") {
    return tmp_nJets >= 4 && mass_sel;
  }
  if (this->getVar() == "nJets") {
    return mass_sel;
  }
  ...
  if (this->getVar() == "jetPt[1]") {
    return tmp_nJets >= 2 && mass_sel;
  }
  if (this->getVar() == "jetEta[0]") {
    return tmp_nJets >= 1 && mass_sel;
  }
  if (this->getVar() == "jetEta[1]") {
    return tmp_nJets >= 2 && mass_sel;
  }
  if (this->getVar() == "absjetEta[0]") {
    return tmp_nJets >= 1 && mass_sel;
  }
  if (this->getVar() == "absjetEta[1]") {
    return tmp_nJets >= 2 && mass_sel;
  }
  ...
  if (this->getVar() == "Mass0jFull") {
    return tmp_nJets == 0 && mass_selFull;
  }
  if (this->getVar() == "Mass1jFull") {
    return tmp_nJets == 1 && mass_selFull;
  }
  if (this->getVar() == "Mass2jFull") {
    return tmp_nJets == 2 && mass_selFull;
  }
  if (this->getVar() == "Mass3jFull") {
    return tmp_nJets == 3 && mass_selFull;
  }
  if (this->getVar() == "Mass34jFull") {
    return tmp_nJets >= 3 && mass_selFull;
  }
  if (this->getVar() == "Mass4jFull") {
    return tmp_nJets >= 4 && mass_selFull;
  }
...
testJets::testJets(const Str& channel, const Str& varName, const Vec<float>& binning) : DijetBranchResponseMatrixMaker<float>(channel, varName, binning) { ; }

SelectedZResponseMatrixMakerBase::SelectedZResponseMatrixMakerBase(const Str& channel, const Str& varName, const Vec<float>& binning, const Str& compVar)
...
UPtr<UMap<size_t, float> > SelectedZResponseMatrixMakerBase::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  ...
  for (size_t row = 0; row < size_t(std::abs(trueTree.GetEntries())); ++row) {
    ...
    (*out)[trueEvt] = (this->z1IsBetter(z1CompTrue, z2CompTrue) ? *z1RespTruePtr : *z2RespTruePtr);
  }

  return std::move(out);
}

void SelectedZResponseMatrixMakerBase::setRecoBranches(TChain& t, const Vec<Str>& objects) {
...
float SelectedZResponseMatrixMakerBase::getEventResponse(const Str& option) const {
  if (this->z1IsBetter(z1CompVar, z2CompVar))
    return *z1RespVarPtr;
  return *z2RespVarPtr;
}

Str SelectedZResponseMatrixMakerBase::constructZVarName(const Str& channel, const Str& var, bool wantZ1) const {
  if (channel == "mmmm") {
    if (wantZ1)
      return "m1_m2_" + var;
    return "m3_m4_" + var;
...
Z1ByMassResponseMatrixMaker::Z1ByMassResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SelectedZResponseMatrixMakerBase(channel, varName, binning, "Mass"), comparisonVarName("Mass") {
  ;
}

bool Z1ByMassResponseMatrixMaker::z1IsBetter(const float z1Comp, const float z2Comp) const { return std::abs(z1Comp - 91.1876) < std::abs(z2Comp - 91.1876); }

Z2ByMassResponseMatrixMaker::Z2ByMassResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SelectedZResponseMatrixMakerBase(channel, varName, binning, "Mass"), comparisonVarName("Mass") {
  ;
}

bool Z2ByMassResponseMatrixMaker::z1IsBetter(const float z1Comp, const float z2Comp) const { return std::abs(z1Comp - 91.1876) > std::abs(z2Comp - 91.1876); }

Z1ByPtResponseMatrixMaker::Z1ByPtResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SelectedZResponseMatrixMakerBase(channel, varName, binning, "Pt"), comparisonVarName("Pt") {
  ;
}

bool Z1ByPtResponseMatrixMaker::z1IsBetter(const float z1Comp, const float z2Comp) const { return z1Comp > z2Comp; }

Z2ByPtResponseMatrixMaker::Z2ByPtResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SelectedZResponseMatrixMakerBase(channel, varName, binning, "Pt"), comparisonVarName("Pt") {
  ;
}

bool Z2ByPtResponseMatrixMaker::z1IsBetter(const float z1Comp, const float z2Comp) const { return z1Comp < z2Comp; }

ZZCompositeResponseMatrixMakerBase::ZZCompositeResponseMatrixMakerBase(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SimpleValueResponseMatrixMakerBase<float>(channel, varName, binning),
...
UPtr<UMap<size_t, float> > ZZCompositeResponseMatrixMakerBase::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, float> > out(new UMap<size_t, float>());
...
void ZZCompositeResponseMatrixMakerBase::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  t.SetBranchAddress(z1VarName.c_str(), &z1InputVar);
  t.SetBranchAddress(z2VarName.c_str(), &z2InputVar);
}

float ZZCompositeResponseMatrixMakerBase::getEventResponse(const Str& option) const { return this->calculateZZVar(z1InputVar, z2InputVar); }

Str ZZCompositeResponseMatrixMakerBase::constructZVarName(const Str& channel, const Str& var, bool wantZ1) const {
  if (channel == "mmmm") {
    if (wantZ1)
      return "m1_m2_" + var;
    return "m3_m4_" + var;
...
ZZDeltaPhiResponseMatrixMaker::ZZDeltaPhiResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : ZZCompositeResponseMatrixMakerBase(channel, "Phi", binning) {
  ;
}

float ZZDeltaPhiResponseMatrixMaker::calculateZZVar(float z1Phi, float z2Phi) const { return ::deltaPhi(z1Phi, z2Phi); }

ZZDeltaRResponseMatrixMaker::ZZDeltaRResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SimpleValueResponseMatrixMakerBase<float>(channel, varName, binning),
...
UPtr<UMap<size_t, float> > ZZDeltaRResponseMatrixMaker::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, float> > out(new UMap<size_t, float>());
...
void ZZDeltaRResponseMatrixMaker::setRecoBranches(TChain& t, const Vec<Str>& objects) {
...
float ZZDeltaRResponseMatrixMaker::getEventResponse(const Str& option) const { return ::deltaR(z1Eta, z1Phi, z2Eta, z2Phi); }

Str ZZDeltaRResponseMatrixMaker::constructZVarName(const Str& channel, const Str& var, bool wantZ1) const {
  if (channel == "mmmm") {
    if (wantZ1)
      return "m1_m2_" + var;
    return "m3_m4_" + var;
...
template <typename T>
MultiBranchResponseMatrixMakerBase<T>::MultiBranchResponseMatrixMakerBase(const Str& channel, const Str& varName, const Vec<float>& binning)
    : VectorValueResponseMatrixMakerBase<T>(channel, varName, binning) {
  ;
}

template <typename T>
UPtr<UMap<size_t, Vec<T> > > MultiBranchResponseMatrixMakerBase<T>::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, Vec<T> > > out(new UMap<size_t, Vec<T> >());

  const Vec<Str> varNames = this->constructVarNames(this->getChannel(), this->getVar());
...
template <typename T>
void MultiBranchResponseMatrixMakerBase<T>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  const Vec<Str> varNames = this->constructVarNames(this->getChannel(), this->getVar());
...
template <typename T>
Vec<T> MultiBranchResponseMatrixMakerBase<T>::getEventResponse(const Str& option) const {
  // copies elements so should be safe
  return values;
}

template <typename T>
Vec<Str> MultiBranchResponseMatrixMakerBase<T>::constructVarNames(const Str& channel, const Str& var) const {
...
AllLeptonBranchResponseMatrixMaker::AllLeptonBranchResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : MultiBranchResponseMatrixMakerBase<float>(channel, varName, binning) {
  ;
}

Vec<Str> AllLeptonBranchResponseMatrixMaker::constructObjectNames(const Str& channel) const {
  if (channel == "mmmm")
...
BothZsBranchResponseMatrixMaker::BothZsBranchResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : MultiBranchResponseMatrixMakerBase<float>(channel, varName, binning) {
  ;
}

Vec<Str> BothZsBranchResponseMatrixMaker::constructObjectNames(const Str& channel) const {
  if (channel == "mmmm")
...
LeptonMaxBranchResponseMatrixMaker::LeptonMaxBranchResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SimpleValueResponseMatrixMakerBase<float>(channel, varName, binning), varNames(constructVarNames(channel, varName)) {
  ;
}

UPtr<UMap<size_t, float> > LeptonMaxBranchResponseMatrixMaker::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, float> > out(new UMap<size_t, float>());
...
void LeptonMaxBranchResponseMatrixMaker::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  values.resize(varNames.size());
...
float LeptonMaxBranchResponseMatrixMaker::getEventResponse(const Str& option) const {
  float max = -999999999.;
...
Vec<Str> LeptonMaxBranchResponseMatrixMaker::constructVarNames(const Str& channel, const Str& var) const {
  Vec<Str> objects;
...
NthJetResponseMatrixMaker<T, _N>::NthJetResponseMatrixMaker(const Str& channel, const Str& varName, const Vec<float>& binning)
    : SimpleValueResponseMatrixMakerBase<T>(channel, varName, binning) {
...
template <typename T, size_t _N>
UPtr<UMap<size_t, T> > NthJetResponseMatrixMaker<T, _N>::getTrueValues(TChain& trueTree, const Vec<Str>& objects, const Str& syst) const {
  UPtr<UMap<size_t, T> > out(new UMap<size_t, T>());
...
template <typename T, size_t _N>
void NthJetResponseMatrixMaker<T, _N>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  allJetValues.clear();
...
template <typename T, size_t _N>
T NthJetResponseMatrixMaker<T, _N>::getEventResponse(const Str& syst) const {
...
template <typename T, size_t _N>
bool NthJetResponseMatrixMaker<T, _N>::selectEvent(const Str& syst) const {
  auto iVal = allJetValues.find(syst);
...
template <class R>
UseSFHists<R>::UseSFHists(const Str& channel, const Str& varName, const Vec<float>& binning) : R(channel, varName, binning) {
...
template <class R>
void UseSFHists<R>::registerElectronSelectionSFHist(const TH2F& h) {
  hEleSelSF.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::registerElectronSelectionGapSFHist(const TH2F& h) {
  hEleSelGapSF.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::registerElectronRecoSFHist(const TH2F& h) {
  hEleRecoSF.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::registerElectronLowRecoSFHist(const TH2F& h) {
  hEleLowRecoSF.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::registerMuonSFHist(const TH2F& h) {
  hMuSF.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::registerMuonSFErrorHist(const TH2F& h) {
  hMuSFErr.reset((TH2F*)h.Clone());
}

template <class R>
void UseSFHists<R>::setupOneLepton(TChain& t, const Str& obj, float*& ptPtr, float& ptVal, float*& etaPtr, float& etaVal, bool*& isGapPtr, bool& isGapVal) {
  Str ptName = obj + "Pt";
...
template <class R>
void UseSFHists<R>::setRecoBranches(TChain& t, const Vec<Str>& objects) {
  ...
  setupOneLepton(t, objects.at(0), lPtsSF.at(0), l1PtSF_value, lEtasSF.at(0), l1EtaSF_value, lIsGapSF.at(0), l1IsGapSF_value);
  setupOneLepton(t, objects.at(1), lPtsSF.at(1), l2PtSF_value, lEtasSF.at(1), l2EtaSF_value, lIsGapSF.at(1), l2IsGapSF_value);
  setupOneLepton(t, objects.at(2), lPtsSF.at(2), l3PtSF_value, lEtasSF.at(2), l3EtaSF_value, lIsGapSF.at(2), l3IsGapSF_value);
  setupOneLepton(t, objects.at(3), lPtsSF.at(3), l4PtSF_value, lEtasSF.at(3), l4EtaSF_value, lIsGapSF.at(3), l4IsGapSF_value);
}

template <class R>
float UseSFHists<R>::getLepSF(const Vec<Str>& leptons, float eSyst, float mSyst) {
  float out = 1.;
  ...
  for (size_t i = 0; i < leptons.size(); ++i) {
    if (leptons.at(i).compare(0, 1, "e") == 0) {
      ...
      if (*lIsGapSF.at(i)) {
        sf *= ::getContentFromHist(*hEleSelGapSF, *lEtasSF.at(i), *lPtsSF.at(i));
        if (eSyst)
          err += ::getErrorFromHist(*hEleSelGapSF, *lEtasSF.at(i), *lPtsSF.at(i));
      } else {
        sf *= ::getContentFromHist(*hEleSelSF, *lEtasSF.at(i), *lPtsSF.at(i));
        if (eSyst)
          err += ::getErrorFromHist(*hEleSelSF, *lEtasSF.at(i), *lPtsSF.at(i));
      }
      if (*lPtsSF.at(i) < 20.) {
        sf *= ::getContentFromHist(*hEleLowRecoSF, *lEtasSF.at(i), *lPtsSF.at(i));
        if (eSyst) {
          float recoErr = ::getErrorFromHist(*hEleLowRecoSF, *lEtasSF.at(i), *lPtsSF.at(i));
          if (*lPtsSF.at(i) < 10.)
            recoErr += 0.01;
          err = eSyst * std::sqrt(err * err + recoErr * recoErr);
        }
      } else {
        sf *= ::getContentFromHist(*hEleRecoSF, *lEtasSF.at(i), *lPtsSF.at(i));
        if (eSyst) {
          float recoErr = ::getErrorFromHist(*hEleRecoSF, *lEtasSF.at(i), *lPtsSF.at(i));
          if (*lPtsSF.at(i) > 75.)
            recoErr += 0.01;
          err = eSyst * std::sqrt(err * err + recoErr * recoErr);
        }
      ...
      float sf = ::getContentFromHist(*hMuSF, *lEtasSF.at(i), *lPtsSF.at(i));
      if (mSyst)
        sf += mSyst * ::getContentFromHist(*hMuSFErr, *lEtasSF.at(i), *lPtsSF.at(i));
...
template <class R>
RelaxGenZCuts<R>::RelaxGenZCuts(const Str& channel, const Str& varName, const Vec<float>& binning) : R(channel, varName, binning) {
  ;
}
