...
def getComLineArgs():
    ...
    parser.add_argument("--thesis", action="store_true", help="For making CMS thesis plots")
    parser.add_argument("--preliminary", action="store_true", help="For making CMS Preliminary plots")
    parser.add_argument("--legend_left", action="store_true", help="Put legend left or right")
    parser.add_argument("--titleOffset", type=float, default=1.0, help="Scale default ymax by this amount")
    ...
    parser.add_argument("--scalelegx", type=float, default=1.0, help="Scale default legend entry wdith by this amount")
    ...
    parser.add_argument("--NormFb", action="store_true", help="Normalize differential cross sections to the luminosity")
...
if EW_corr and EW_P4:
    ...
    bmg = 0.45
    bpwidth = 0.11
lastbpw = round(bpwidth / (1.0 - bmg), 2)
...
if not include_MiNNLO:
    ...
    if EW_P4 and EW_corr:
        panel_breakpoints = [0.01, 0.01 + lastbpw] + [
            round(0.01 + lastbpw + bpwidth * bpind, 2) for bpind in range(1, 4)
        ]
...
for var, prettyVar in prettyVars.iteritems():
    ...
    if yaxisunits[var]:
        ...
        yt = _yTitleTemp.format(
            ...
            units=f"\\, \\left( \\frac{{1}}{{\\text{{{yaxisunits[var]}}}}} \\right)",
        )
        ytnn = _yTitleTemp.format(
            xvar=prettyVar,
            prefix="",
            units=f"\\, \\left( \\frac{{\\text{{fb}}}}{{\\text{{{yaxisunits[var]}}}}} \\right)",
        )
    else:
    ...
    _yTitle[var] = yt
    if var == "dEtajj":
        _yTitle[var] = _yTitleTemp.format(
            xvar="\\vert \\Delta \\eta (j_1,j_2) \\vert", prefix="\\frac{1}{\\sigma_{\\text{fid}}}", units=""
        )
    if var == "mjj":
        _yTitle[var] = _yTitleTemp.format(
            xvar="m_{jj}",
            prefix="\\frac{1}{\\sigma_{\\text{fid}}}",
            units="\\, \\left( \\frac{{1}}{{\\text{{{unit}}}}} \\right)".format(unit="GeV"),
        )
...
legParams["deltaEtajj"]["leftmargin"] = 0.5
legParams["deltaEtajj"]["rightmargin"] = 0.03
legParams["deltaEtajj"]["topmargin"] = 0.05
...
def createRatio(h1, h2):
    ...
    for i in range(1, Nbins + 1):
        ...
        if datacontent != 0:
            ...
            error = 0.0  # set to 0 just in case, 0 bin cont shouldn't be drawn with PE1 option #2.07 #why 2.07?
    ...
    line = ROOT.TLine(h1.GetXaxis().GetXmin(), 1.0, h1.GetXaxis().GetXmax(), 1.0)
    line.SetLineStyle(7)
...
def getPrettyLegend(hTrue, data_hist, hAltTrue, error_hist, coords, hTrueNNLO=None, hTrueEWC=None):
    ...
    legend.AddEntry(
        hAltTrue, sigLabelAlt, "lep"
    )  # intended to use "le" but this makes the legend ugly in pdf, but with "lep" looks ok
    if include_MiNNLO:
...
def getRYaxis(hUnf1, ratioErrorBand1, lastP):
    ...
    Ryaxis = ROOT.TGaxis(
        hUnf1.GetXaxis().GetXmax(),
        ratioErrorBand1.GetMinimum(),
        hUnf1.GetXaxis().GetXmax(),
        ratioErrorBand1.GetMaximum(),
        ratioErrorBand1.GetMinimum(),
        ratioErrorBand1.GetMaximum(),
        3,
        "+CS",
    ...
    Ryaxis.SetLabelSize(0.0)  # 0.1485
    Ryaxis.SetTitleFont(42)
    Ryaxis.SetTitleSize(0.0)
...
def RatioErrorBand(Ratio, hUncUp, hUncDn, hTrueNoErrs, varName):
    ...
    for i in range(1, tmpData.GetNbinsX() + 1):
        ...
        errorDn = max(
            tmpData.GetBinContent(i) - math.sqrt(math.pow(tmpData.GetBinError(i), 2) + math.pow((eDn / tru), 2)), 0
        )
...
def generatePlots(
    ...
    lumifb = lumi
    print(
        "======================hUnf Integral before normalization: %s========================"
        % hUnf.Integral(1, hUnf.GetNbinsX())
    )
    if norm:
    ...
    if unfoldDir:
        ...
        if not norm and normFb:
            print(f"Inclusive fiducial cross section = {hUnf.Integral(1, hUnf.GetNbinsX())} fb")
        if norm or normFb:
            normalizeBins(hUnf)
        ...
        xsecfac = 40.5
        print(
            "==========Info: Unfolded bin content, total uncup,uncdn, and MiNNLO bin content and bin err====================="
        )
        print([xsecfac * hUnf.GetBinContent(infotmp) for infotmp in range(1, hUnf.GetNbinsX() + 1)])
        print(
            [
                xsecfac * math.sqrt((hUncUp.GetBinContent(infotmp)) ** 2 + (hUnf.GetBinError(infotmp)) ** 2)
                for infotmp in range(1, hUnf.GetNbinsX() + 1)
            ]
        )
        print(
            [
                xsecfac * math.sqrt((hUncDn.GetBinContent(infotmp)) ** 2 + (hUnf.GetBinError(infotmp)) ** 2)
                for infotmp in range(1, hUnf.GetNbinsX() + 1)
            ]
        )
        if include_MiNNLO:
        ...
        UnfErrBand.Draw(
            error_drawopt
        )  # somehow have to draw "a2" first to draw the frame, then draw with the desired option
        ...
        offset = ROOT.gPad.GetLeftMargin() - 0.07 if args["legend_left"] else ROOT.gPad.GetRightMargin() - 0.07
        width = 0.33
        ...
        if varName == "drz1z2":
            xdist = 0.07
        xcoords = (
            [xdist + offset, xdist + width + offset]
            if args["legend_left"]
        ...
        Altyaxis = ROOT.TGaxis(
            hUnf.GetXaxis().GetXmin(),
            ratioErrorBand.GetMinimum(),
            hUnf.GetXaxis().GetXmin(),
            ratioErrorBand.GetMaximum(),
            ratioErrorBand.GetMinimum(),
            ratioErrorBand.GetMaximum(),
            3,
            "CS",
        ...
        if include_MiNNLO:
            ...
            if not (EW_corr and EW_P4):
                NNLOyaxis.SetTickLength(yrtl / (1.0 - bmg))
            # axText3=getAxisTextBox(0.06,0.0,"Data/%s"%ratioName_alt,0.23,True)
...
for varName in runVariables:
    ...
    if args["makeTotals"]:
        ...
        if not include_MiNNLO:
            generatePlots(
                hUnfTot, hTotUncUp, hTotUncDn, hTrueTot, hTrueAltTot, varName, norm, normFb, args["lumi"], UnfoldOutDir
            )
        else:
            if not EW_corr:
                generatePlots(
