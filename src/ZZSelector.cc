...
void ZZSelector::Init(TTree* tree) {
  ...
  hists1D_ = {
      "yield", "ZMass", "ZZPt", "dPhiZ1Z2", "ZPt", "LepPt", "LepEta", "Mass", "nJets", "jetPt[0]", "jetPt[1]", "jetEta[0]", "jetEta[1]", "mjj", "dEtajj"};
...
void ZZSelector::LoadBranchesUWVV(Long64_t entry, std::pair<Systematic, std::string> variation) {
  ...
  if (variation.first == Central) {
    ...
    if (variation.first == electronEfficiencyUp || variation.first == electronEfficiencyDown || variation.first == electronRecoEffUp ||
        variation.first == electronRecoEffDown || variation.first == muonEfficiencyUp || variation.first == muonEfficiencyDown) {
