...
from rootpy.plotting import Canvas, Graph, Hist, HistStack
...
from rootpy.ROOT import TBox, TDecompSVD, TLatex, TLegend
...
from Analysis.weightHelpers import baseMCWeight, puWeight

from PlotTools import PlotStyle as _Style
from PlotTools import addPadsBelow, fixRatioAxes, makeErrorBand, makeLegend, makeRatio
from PlotTools import pdfViaTex as _pdfViaTex
from Utilities import Z_MASS, combineWeights, deltaPhiString, deltaRString, zeroNegativeBins
...
_variables = {
    "pt": dict.fromkeys(_channels, "Pt"),
    "mass": dict.fromkeys(_channels, "Mass"),
    "massFull": dict.fromkeys(_channels, "Mass"),
    "eta": dict.fromkeys(_channels, "abs(Eta)"),
    "z1Mass": {"eeee": "e1_e2_Mass", "mmmm": "m1_m2_Mass", "eemm": ["e1_e2_Mass", "m1_m2_Mass"]},
    "z2Mass": {"eeee": "e3_e4_Mass", "mmmm": "m3_m4_Mass", "eemm": ["m1_m2_Mass", "e1_e2_Mass"]},
    "z1Pt": {"eeee": "e1_e2_Pt", "mmmm": "m1_m2_Pt", "eemm": ["e1_e2_Pt", "m1_m2_Pt"]},
    "z2Pt": {"eeee": "e3_e4_Pt", "mmmm": "m3_m4_Pt", "eemm": ["m1_m2_Pt", "e1_e2_Pt"]},
    "zHigherPt": {"eeee": ["e1_e2_Pt", "e3_e4_Pt"], "mmmm": ["m1_m2_Pt", "m3_m4_Pt"], "eemm": ["e1_e2_Pt", "m1_m2_Pt"]},
    "zLowerPt": {"eeee": ["e1_e2_Pt", "e3_e4_Pt"], "mmmm": ["m1_m2_Pt", "m3_m4_Pt"], "eemm": ["e1_e2_Pt", "m1_m2_Pt"]},
    "zPt": {"eeee": ["e1_e2_Pt", "e3_e4_Pt"], "mmmm": ["m1_m2_Pt", "m3_m4_Pt"], "eemm": ["e1_e2_Pt", "m1_m2_Pt"]},
    "deltaPhiZZ": {
        "eeee": f"abs({deltaPhiString()}(e1_e2_Phi, e3_e4_Phi))",
        "eemm": f"abs({deltaPhiString()}(e1_e2_Phi, m1_m2_Phi))",
        "mmmm": f"abs({deltaPhiString()}(m1_m2_Phi, m3_m4_Phi))",
    },
    "deltaRZZ": {
        "eeee": f"{deltaRString()}(e1_e2_Eta, e1_e2_Phi, e3_e4_Eta, e3_e4_Phi)",
        "eemm": f"{deltaRString()}(e1_e2_Eta, e1_e2_Phi, m1_m2_Eta, m1_m2_Phi)",
        "mmmm": f"{deltaRString()}(m1_m2_Eta, m1_m2_Phi, m3_m4_Eta, m3_m4_Phi)",
    },
    "lPt": {
...
_binning = {
    ...
    "nJets": [5, -0.5, 4.5],
    "mass": [100.0] + [200.0 + 50.0 * i for i in range(5)] + [500.0, 600.0, 800.0],
    "massFull": [80.0, 100.0, 120.0, 130.0, 150.0, 180.0, 200.0, 240.0, 300.0, 400.0, 1000],
    "eta": [6, 0.0, 6.0],
    "jet1Pt": [0.0, 50.0, 100.0, 200.0, 300.0, 500.0],
    "jet1Eta": [0.0, 1.5, 3.0, 4.7],
    "jet2Pt": [30.0, 100.0, 200.0, 500.0],
    "jet2Eta": [0.0, 1.5, 3.0, 4.7],
    "mjj": [0.0, 100.0, 300.0, 800.0],
    "deltaEtajj": [6, 0.0, 6.0],
    ...
    "deltaPhiZZ": [0.0, 1.5] + [2.0 + 0.25 * i for i in range(6)],
    "deltaRZZ": [6, 0.0, 6.0],
    "lPt": [15, 0.0, 150.0],
...
for var, prettyVar in _prettyVars.iteritems():
    ...
    if _units[var]:
        ...
        yt = _yTitleTemp.format(
            ...
            units=f"\\, \\left( \\frac{{1}}{{\\text{{{_units[var]}}}}} \\right)",
        )
        ytnn = _yTitleTemp.format(
            xvar=prettyVar,
            prefix="",
            units=f"\\, \\left( \\frac{{\\text{{fb}}}}{{\\text{{{_units[var]}}}}} \\right)",
        )
    else:
...
_selections = {
    "pt": dict.fromkeys(_channels, ""),
    "mass": dict.fromkeys(_channels, ""),
    "massFull": dict.fromkeys(_channels, ""),
    "eta": dict.fromkeys(_channels, ""),
    "z1Mass": {
        "eeee": "",
        "mmmm": "",
        "eemm": [
            f"abs(e1_e2_Mass - {Z_MASS}) < abs(m1_m2_Mass - {Z_MASS})",
            f"abs(e1_e2_Mass - {Z_MASS}) > abs(m1_m2_Mass - {Z_MASS})",
        ],
    },
    "z2Mass": {
    ...
    "deltaPhiZZ": dict.fromkeys(_channels, ""),
    "deltaRZZ": dict.fromkeys(_channels, ""),
    "lPt": dict.fromkeys(_channels, ""),
    "l1Pt": dict.fromkeys(_channels, ""),
}
...
for sys in ["", "_jerUp", "_jerDown", "_jesUp", "_jesDown"]:
    for varName in ["nJets", "mjj", "deltaEtajj"]:
        ...
        _variables[varName] = dict.fromkeys(_channels, var)

        if "jj" in varName.lower():
            _selections[varName] = dict.fromkeys(_channels, f"nJets{sys} > 1")
        else:
            _selections[varName] = dict.fromkeys(_channels, "")
    ...
    for baseVar in ["jetPt" + sys, "jetEta" + sys]:
        for j in [1, 2]:
            ...
            if doAbs:
                var = f"abs({var})"
            varName = baseVar.replace("jet", f"jet{j}")

            _variables[varName] = dict.fromkeys(_channels, var)
            _selections[varName] = dict.fromkeys(_channels, f"nJets{sys} >= {j}")
...
_trueSelections["massFull"] = dict.fromkeys(_channels, "")
...
_responseClassNames = {
    "mass": dict.fromkeys(_channels, "FloatBranchResponseMatrixMaker"),
    "massFull": dict.fromkeys(_channels, "FullSpectrumFloatResponseMatrixMaker"),
    "pt": dict.fromkeys(_channels, "FloatBranchResponseMatrixMaker"),
    "eta": dict.fromkeys(_channels, "AbsFloatBranchResponseMatrixMaker"),
    "nJets": dict.fromkeys(_channels, "JetUIntBranchResponseMatrixMaker"),
    "mjj": dict.fromkeys(_channels, "DijetBranchResponseMatrixMaker"),
    "deltaEtajj": dict.fromkeys(_channels, "AbsDijetBranchResponseMatrixMaker"),
    "z1Mass": {
    ...
    "zHigherPt": dict.fromkeys(_channels, "Z1ByPtResponseMatrixMaker"),
    "zLowerPt": dict.fromkeys(_channels, "Z2ByPtResponseMatrixMaker"),
    "deltaPhiZZ": dict.fromkeys(_channels, "ZZAbsDeltaPhiResponseMatrixMaker"),
    "deltaRZZ": dict.fromkeys(_channels, "ZZDeltaRResponseMatrixMaker"),
    "lPt": dict.fromkeys(_channels, "AllLeptonBranchResponseMatrixMaker"),
    "l1Pt": dict.fromkeys(_channels, "LeptonMaxBranchResponseMatrixMaker"),
    "zPt": dict.fromkeys(_channels, "BothZsBranchResponseMatrixMaker"),
    "jet1Pt": dict.fromkeys(_channels, "FirstJetFloatResponseMatrixMaker"),
    "jet2Pt": dict.fromkeys(_channels, "SecondJetFloatResponseMatrixMaker"),
    "jet1Eta": dict.fromkeys(_channels, "FirstJetAbsFloatResponseMatrixMaker"),
    "jet2Eta": dict.fromkeys(_channels, "SecondJetAbsFloatResponseMatrixMaker"),
}
...
_varNamesForResponseMaker = {
    ...
    "massFull": dict.fromkeys(_channels, "Mass"),
    "pt": dict.fromkeys(_channels, "Pt"),
    "eta": dict.fromkeys(_channels, "Eta"),
    "nJets": dict.fromkeys(_channels, "nJets"),
    "mjj": dict.fromkeys(_channels, "mjj"),
    "deltaEtajj": dict.fromkeys(_channels, "deltaEtajj"),
    "z1Mass": {
        "eeee": "e1_e2_Mass",
        "mmmm": "m1_m2_Mass",
        "eemm": "Mass",
    ...
    "zHigherPt": dict.fromkeys(_channels, "Pt"),
    "zLowerPt": dict.fromkeys(_channels, "Pt"),
    "deltaPhiZZ": dict.fromkeys(_channels, "deltaPhiZZ"),
    ...
    "jet2Eta": dict.fromkeys(_channels, "jetEta"),
}
...
_legParams["deltaEtajj"]["leftmargin"] = 0.5
_legParams["deltaEtajj"]["rightmargin"] = 0.03
_legParams["deltaEtajj"]["topmargin"] = 0.05
...
_legParamsLogy["deltaRZZ"]["leftmargin"] = 0.18
_legParamsLogy["deltaRZZ"]["rightmargin"] = 0.22
...
_cacheFileTemplate = _join(_env["zzt"], "Analysis", "savedResults", "unfoldCache_{}Iter.root")
...
def _getUnfolded(hSig, hBkg, hTrue, hResponse, hData, nIter, withRespAndCov=False, printIt=False):
    ...
    try:
        ...
        print(
            f"Generating debug plots sig{_printCounter}.png, bkg{_printCounter}.png, true{_printCounter}.png, data{_printCounter}.png, resp{_printCounter}.png, resp{_printCounter}.root"
        )
        c = Canvas(1000, 1000)
        hSig.draw()
        _style.setCMSStyle(c, "", dataType="Debug", intLumi=35860.0)
        c.Print(f"sig{_printCounter}.png")
        hBkg.draw()
        _style.setCMSStyle(c, "", dataType="Debug", intLumi=35860.0)
        c.Print(f"bkg{_printCounter}.png")
        hTrue.draw()
        _style.setCMSStyle(c, "", dataType="Debug", intLumi=35860.0)
        c.Print(f"true{_printCounter}.png")
        hData.draw()
        _style.setCMSStyle(c, "", dataType="Debug", intLumi=35860.0)
        c.Print(f"data{_printCounter}.png")
        hResponse.drawstyle = "colz"
        hResponse.draw()
        _style.setCMSStyle(c, "", dataType="Debug", intLumi=35860.0)
        c.Print(f"resp{_printCounter}.png")
        c.Print(f"resp{_printCounter}.root")
        _printCounter += 1
...
def _generateAnalysisInputs(puWeightFile, looseSIP=False, noSIP=False, sfRemake=False):
    sfFiles = {}
    sipForBkg = 4.0
    if noSIP:
        ...
        sipForBkg = -1
        if looseSIP:
            raise ValueError("You can use scale factors for loose SIP cut or no SIP cut, but not both.")
    elif looseSIP:
    ...
    with preserve_current_directory(), root_open(puWeightFileFull) as fPU:
        hPUWt = {
    ...
    if sfFiles:
        with preserve_current_directory():
            with root_open(
                _join(_env["zzt"], "data", "leptonScaleFactors", sfFiles["eSelSFFile"] + ".root")
            ) as fEleSel:
                hSF["eSel"] = asrootpy(fEleSel.EGamma_SF2D).clone()
                hSF["eSel"].SetDirectory(0)
            with root_open(
                _join(_env["zzt"], "data", "leptonScaleFactors", sfFiles["eSelSFFileGap"] + ".root")
            ) as fEleSelGap:
                hSF["eSelGap"] = asrootpy(fEleSelGap.EGamma_SF2D).clone()
                hSF["eSelGap"].SetDirectory(0)
            with root_open(
                _join(_env["zzt"], "data", "leptonScaleFactors", sfFiles["eRecoSFFile"] + ".root")
            ) as fEleReco:
                hSF["eReco"] = asrootpy(fEleReco.EGamma_SF2D).clone()
                hSF["eReco"].SetDirectory(0)
            with root_open(_join(_env["zzt"], "data", "leptonScaleFactors", sfFiles["mSFFile"] + ".root")) as fMuSF:
...
def _generateSamples(inData, inMC, ana, fakeRateFile, puWeightFile, lumi, amcatnlo=False, sipForBkg=4.0, sfFiles={}):
    ...
    allSamples["true"] = genZZSamples("zz", inMC, ana, lumi, amcatnlo=amcatnlo, higgs=(ana == "full"))
    allSamples["reco"] = zzStackSignalOnly(
        "zz", inMC, ana, puWeightFile, lumi, amcatnlo=amcatnlo, asGroup=True, higgs=(ana == "full"), **sfFiles
    )

    allSamples["bkgMC"] = zzIrreducibleBkg("zz", inMC, ana, puWeightFile, lumi, **sfFiles)
    allSamples["bkg"] = standardZZBkg("zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, sipCut=sipForBkg)
    allSamples["bkgSyst"] = {
        "eup": standardZZBkg(
            "zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, eFakeRateSyst="up", sipCut=sipForBkg
        ),
        "edn": standardZZBkg(
            "zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, eFakeRateSyst="dn", sipCut=sipForBkg
        ),
        "mup": standardZZBkg(
            "zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, mFakeRateSyst="up", sipCut=sipForBkg
        ),
        "mdn": standardZZBkg(
            "zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, mFakeRateSyst="dn", sipCut=sipForBkg
        ),
    }

    allSamples["data"] = standardZZData("zz", inData, ana)

    allSamples["altReco"] = zzStackSignalOnly(
        "zz", inMC, ana, puWeightFile, lumi, amcatnlo=(not amcatnlo), asGroup=True, higgs=(ana == "full"), **sfFiles
    )

    allSamples["altTrue"] = genZZSamples("zz", inMC, ana, lumi, amcatnlo=(not amcatnlo), higgs=(ana == "full"))
    allSamples["recoSyst"] = {}
    allSamples["bkgMCSyst"] = {}
    sigFileNamesSyst = {}
    for syst in ["eScaleUp", "eScaleDn", "eRhoResUp", "eRhoResDn", "ePhiResUp"]:
        allSamples["recoSyst"][syst] = zzStackSignalOnly(
            "eeee,eemm",
            inMC.replace("mc_", f"mc_{syst}_"),
            ana,
            puWeightFile,
            lumi,
            amcatnlo=amcatnlo,
        ...
        allSamples["bkgMCSyst"][syst] = zzIrreducibleBkg(
            "eeee,eemm", inMC.replace("mc_", f"mc_{syst}_"), ana, puWeightFile, lumi, **sfFiles
        )
...
def _unfold(varName, chan, samples, puWeightFile, sfFiles, responseMakers, altResponseMakers, nIter, plotDir=""):
    # outputs
    hUnfolded = {}
    hTrue = {}
    hTrueAlt = {}
    ...
    nominalWeight = baseMCWeight(chan, puWeightFile, **sfFiles)
    ...
    hTrue[""] = samples["true"][chan].makeHist(var, selTrue, binning, perUnitWidth=False)
    ...
    hUnfolded[""], hCov, hResp = _getUnfolded(
        hSigNominal, hBkgMCNominal + hBkgNominal, hTrue[""], hResponseNominalTotal, hData, nIter, True
    )
    ...
    if plotDir:
        ...
        hResp.draw()
        _style.setCMSStyle(cRes, "", dataType="Internal", intLumi=35860.0)
        cRes.Print(_join(plotDir, "pngs", f"response_{varName}_{chan}.png"))
        cRes.Print(_join(plotDir, "Cs", f"response_{varName}_{chan}.C"))
        ...
        if varName == "massFull":
            ...
            cCov.SetLogy()
        hCov.Draw("colztext")
        _style.setCMSStyle(cCov, "", dataType="Internal", intLumi=35860.0)
        cCov.Print(_join(plotDir, "pngs", f"covariance_{varName}_{chan}.png"))
        cCov.Print(_join(plotDir, "Cs", f"covariance_{varName}_{chan}.C"))

    # PU reweight uncertainty
    for sys in ["up", "dn"]:
        wtStr = baseMCWeight(chan, puWeightFile, puSyst=sys, **sfFiles)
        ...
        hUnfolded["pu_" + sys] = _getUnfolded(hSig, hBkgMC + hBkgNominal, hTrue[""], hResponse, hData, nIter)
    ...
    for lep in set(chan):
        for sys in ["up", "dn"]:
            ...
            hUnfolded[lep + "Eff_" + sys] = _getUnfolded(hSig, hBkgMC + hBkgNominal, hTrue[""], hResponse, hData, nIter)
    ...
    hUnfolded["generator"] = _getUnfolded(hSig, hBkgMCNominal + hBkgNominal, hTrueAlt[""], hResponse, hData, nIter)
    ...
    for sys, scale in lumiScale.iteritems():
        ...
        hUnfolded["lumi_" + sys] = _getUnfolded(hSig, hBkgMC + hBkgNominal, hTrueLumiShift, hResponse, hData, nIter)

    # lepton fake rate uncertainty
    for lep in set(chan):
        for sys in ["up", "dn"]:
            hBkg = samples["bkgSyst"][lep + sys][chan].makeHist(var, sel, binning, perUnitWidth=False, postprocess=True)

            hUnfolded[lep + "FR_" + sys] = _getUnfolded(
                hSigNominal, hBkgMCNominal + hBkg, hTrue[""], hResponseNominalTotal, hData, nIter
            )
    ...
    if "jet" in varName.lower() or "jj" in varName.lower():
        for shift in ["up", "dn"]:
            ...
            for sys in ["jer", "jes"]:
                ...
                hSig = samples["reco"][chan].makeHist(varShifted, selShifted, binning, perUnitWidth=False)
                hBkgMC = samples["bkgMC"][chan].makeHist(varShifted, selShifted, binning, perUnitWidth=False)

                hResponse = sum(asrootpy(resp(sys + "_" + shift)) for resp in responseMakers.values())

                hUnfolded[sys + "_" + shift] = _getUnfolded(
                    hSig, hBkgMC + hBkgNominal, hTrue[""], hResponse, hData, nIter
                )
    ...
    if "e" in chan:
        for sys in ["eScale", "eRhoRes", "ePhiRes"]:
            for shift in ["up", "dn"]:
                ...
                hSig = samples["recoSyst"][sys + sysStr][chan].makeHist(var, sel, binning, perUnitWidth=False)
                hBkgMC = samples["bkgMCSyst"][sys + sysStr][chan].makeHist(var, sel, binning, perUnitWidth=False)
                hResponse = sum(asrootpy(resp(sys + "_" + shift)) for resp in responseMakers.values())

                storeAs = sys + "_" + shift
                if sys == "ePhiRes":
                    storeAs = sys
                hUnfolded[storeAs] = _getUnfolded(hSig, hBkgMC + hBkgNominal, hTrue[""], hResponse, hData, nIter)
    if "m" in chan:
        sys = "mClosure"
        for shift in ["up", "dn"]:
            sysStr = "Up" if shift == "up" else "Dn"
            ...
            hUnfolded[sys + "_" + shift] = _getUnfolded(hSig, hBkgMC + hBkgNominal, hTrue[""], hResponse, hData, nIter)
    ...
    hUnfolded["pdf_up"] = _getUnfolded(hSigUp, hBkgMCNominal + hBkgNominal, hTrue["pdf_up"], hResponseUp, hData, nIter)
    hUnfolded["pdf_dn"] = _getUnfolded(hSigDn, hBkgMCNominal + hBkgNominal, hTrue["pdf_dn"], hResponseDn, hData, nIter)

    # QCD scale uncertainties
    variationIndices = [1, 2, 3, 4, 6, 8]
    hSigs = [
        samples["reco"][chan].makeHist(
            var,
            sel,
            binning,
            {
                "ZZTo4L": f"scaleWeights[{i}]/scaleWeights[0]",
                "ZZTo4L-amcatnlo": f"scaleWeights[{i}]/scaleWeights[0]",
                "ZZJJTo4L_EWK": f"scaleWeights[{i}]/scaleWeights[0]",
            },
            perUnitWidth=False,
        )
    ...
    for bUp, bDn, variations in zip(hTrue["scale_up"], hTrue["scale_dn"], zip(*hTrues)):
    ...
    for bUp, bDn, variations in zip(hTrueAlt["scale_up"], hTrueAlt["scale_dn"], zip(*hTruesAlt)):
        bUp.value = max(b.value for b in variations)
        bDn.value = min(b.value for b in variations)
    ...
    for hSig, hTr, hResponse in zip(hSigs, hTrues, hResponseVariations):
        hUnfoldedVariations.append(_getUnfolded(hSig, hBkgMCNominal + hBkgNominal, hTr, hResponse, hData, nIter))
    ...
    hSigs = [
        samples["reco"][chan].makeHist(
            ...
            binning,
            {
                "ZZTo4L": f"pdfWeights[{i}]/pdfWeights[0]",
                "ZZTo4L-amcatnlo": f"pdfWeights[{i}]/pdfWeights[0]",
                "ZZJJTo4L_EWK": f"pdfWeights[{i}]/pdfWeights[0]",
            },
            perUnitWidth=False,
        )
    ...
    hUnfUp = _getUnfolded(hSigs[0], hBkgNominal + hBkgMCNominal, hTrues[0], hResponses[0], hData, nIter)
    hUnfDn = _getUnfolded(hSigs[1], hBkgNominal + hBkgMCNominal, hTrues[1], hResponses[1], hData, nIter)
    ...
    for sys, shift in mcfmUnc.iteritems():
        ...
        hTrueAlt["mcfmxsec_" + sys] = samples["altTrue"][chan].makeHist(
            var, selTrue, binning, {"GluGluZZ": str(1.0 + shift)}, perUnitWidth=False
        )
        ...
        hUnfolded["mcfmxsec_" + sys] = _getUnfolded(
            hSig, hBkgNominal + hBkgMCNominal, hTrue["mcfmxsec_" + sys], hResponse, hData, nIter
        )
...
def _generateUncertainties(hDict, norm, **plotArgs):
    ...
    if plot:
        lumi = plotArgs.get("lumi", 35860.0)
    ...
    if plot:
        ...
        if varName == "deltaRZZ":
            if chan == "eeee":
                drawOpts["ylimits"] = (0.0, 140.0)
            elif chan == "mmmm":
                drawOpts["ylimits"] = (0.0, 75.0)
            else:
                drawOpts["ylimits"] = (0.0, 58.0)
        ...
        cErrUp.Print(_join(plotDir, "pngs", f"errUp_{varName}_{chan}.png"))
        cErrUp.Print(_join(plotDir, "Cs", f"errUp_{varName}_{chan}.C"))
        ...
        cErrDn.Print(_join(plotDir, "pngs", f"errDown_{varName}_{chan}.png"))
        cErrDn.Print(_join(plotDir, "Cs", f"errDown_{varName}_{chan}.C"))
...
def _generatePlots(
    ...
    if not norm:
        print(f"Inclusive {chan} fiducial cross section = {hUnf.Integral(0, hUnf.GetNbinsX() + 1)} fb")
    _normalizeBins(hUnf)
    ...
    hTrueLeg.linecolor = (
        "#7f7fff"  # color transparent error band looks like on white background, found with GIMP eyedropper
    )
    ...
    trueLegOverlay = Hist(1, 0, 1, title="", legendstyle="L", linecolor=hTrueDraw.linecolor)
    ...
    hTrueDrawAlt.title = f"{signalNameAlt}"
    ...
    trueLegOverlayAlt = Hist(
        1, 0, 1, linecolor=hTrueDrawAlt.linecolor, legendstyle="L", title="", linestyle=hTrueDrawAlt.linestyle
    )
    ...
    if varName in _matrixNames:
        ...
        matDistLegOverlay = Hist(
            1,
            0,
            1,
            linecolor=matDist.linecolor,
            legendstyle="L",
            title="",
    ...
    mainPad.cd()
    (xaxis, yaxis), (xmin, xmax, ymin, ymax) = draw(toPlot, mainPad, **drawOpts)
    ...
    latex.SetTextSize(0.16)  # 3)
    latex.SetTextFont(62)
    latexXMargin = 0.15
    if varName in ["deltaRZZ", "massFull"]:
        latex.SetTextAlign(31)
        latexXMargin = 1.0 - latexXMargin
    elif varName == "l1Pt":
        latexXMargin = 0.35
    ...
    if varName in _matrixNames:
        ...
        for b in matUnity:
            b.value = 1.0
        ratioTheoryErrorMat = makeErrorBand(matUnity, matDistUp / matNoErrs, matDistDn / matNoErrs)
        ratioTheoryErrorMat.fillstyle = "solid"
        ratioTheoryErrorMat.SetFillColorAlpha(819, 0.5)

        ratioErrorMat = makeErrorBand(hUnf / matNoErrs, hUncUp / matNoErrs, hUncDn / matNoErrs)
        (ratioMatX, ratioMatY), ratioMatLimits = draw(
            [ratioTheoryErrorMat, ratioErrorMat, ratioMat], ratioPadMat, **drawOpts
        )
    ...
    for b in hTrueNoErrs:
        b.error = 0

    ratioMain, unityMain = makeRatio(hUnf, hTrueNoErrs)
    ratioErrorMain = makeErrorBand(hUnf / hTrueNoErrs, hUncUp / hTrueNoErrs, hUncDn / hTrueNoErrs)

    hTrueUnity = hTrueDraw.clone()  # need errors only as baseline for ratio theory uncertainties
    for b in hTrueUnity:
        b.value = 1.0
    ratioTheoryError = makeErrorBand(hTrueUnity, hTrueUncUp / hTrueNoErrs, hTrueUncDn / hTrueNoErrs)
    ratioTheoryError.fillstyle = "solid"
    ratioTheoryError.SetFillColorAlpha(600, 0.5)

    (ratioMainX, ratioMainY), ratioMainLimits = draw(
        [ratioTheoryError, ratioErrorMain, ratioMain], ratioPadMain, **drawOpts
    )
    ...
    for b in hTrueNoErrsAlt:
        b.error = 0

    ratioAlt, unityAlt = makeRatio(hUnf, hTrueNoErrsAlt)
    ratioErrorAlt = makeErrorBand(hUnf / hTrueNoErrsAlt, hUncUp / hTrueNoErrsAlt, hUncDn / hTrueNoErrsAlt)

    hTrueUnityAlt = hTrueDrawAlt.clone()  # need errors only as baseline for ratio theory uncertainties
    for b in hTrueUnityAlt:
        b.value = 1.0
    ratioTheoryErrorAlt = makeErrorBand(hTrueUnityAlt, hTrueUncUpAlt / hTrueNoErrsAlt, hTrueUncDnAlt / hTrueNoErrsAlt)
    ratioTheoryErrorAlt.fillstyle = "solid"
    ratioTheoryErrorAlt.SetFillColorAlpha(628, 0.5)

    (ratioAltX, ratioAltY), ratioAltLimits = draw(
        [ratioTheoryErrorAlt, ratioErrorAlt, ratioAlt], ratioPadAlt, **drawOpts
    )
    ratioAltY.CenterTitle()
    unityAlt.Draw("same")
    latex.SetTextSize(latex.GetTextSize() * ratioPadMain.height / ratioPadAlt.height)
    latex.DrawLatex(latexXMargin, 1.0 - 0.2 * ratioPadMain.height / ratioPadAlt.height, signalNameAlt)
    ...
    cUnf.Print(_join(plotDir, "pngs", f"unfold_{varName}{suffix}.png"))
    cUnf.Print(_join(plotDir, "Cs", f"unfold_{varName}{suffix}.C"))
    _pdfViaTex(cUnf, f"unfold_{varName}{suffix}", _join(plotDir, "texs"), _join(plotDir, "pdfs"))
    ...
    **kwargs,
    ...
    for fileType in (
        ...
        if not _exists(subDir):
            ...
            raise OSError(f"There is already some non-directory object called {subDir}.")
    ...
    for varName in varNames:
        ...
        for chan in channels[::-1]:
            ...
            if needToCreate:
                if samples is None:
                    sfFiles, hPUWt, hSF, sipForBkg = _generateAnalysisInputs(puWeightFile, looseSIP, noSIP, sfRemake)
                    samples = _generateSamples(
                        inData, inMC, ana, fakeRateFile, puWeightFile, lumi, amcatnlo, sipForBkg, sfFiles
                    )

                responseMakers, altResponseMakers = _generateResponseClass(varName, chan, samples, hPUWt, hSF)

                hUnfolded[chan], hTrue[chan], hTrueAlt[chan] = _unfold(
                    varName, chan, samples, puWeightFile, sfFiles, responseMakers, altResponseMakers, nIter, plotDir
                )
            ...
            hErr[chan] = _generateUncertainties(
                hUnfolded[chan], norm, lumi=lumi, varName=varName, ana=ana, plotDir=plotDir, chan=chan
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=("Name of fake rate file (assumed to be in usual data directory unless full path is specified)"),
    )
    parser.add_argument(
        "--puWeightFile",
        type=str,
        ...
        help=("Name of pileup weight file (assumed to be in usual data directory unless full path is specified)"),
    )
    parser.add_argument(
        "--lumi", type=float, nargs="?", default=35900.0, help="Integrated luminosity of sample (in pb^-1)"
    )
    parser.add_argument("--nIter", type=int, nargs="?", default=4, help="Number of iterations for D'Agostini method")
    parser.add_argument(
        "--amcatnlo",
        action="store_true",
        help="Use MadGraph5_aMC@NLO as the primary MC and Powheg as the cross-check, instead of the other way around.",
    )
    parser.add_argument(
        "--analysis", "--ana", type=str, nargs="?", default="smp", help="Which set of cuts to use (full, smp, etc.)."
    )
    parser.add_argument(
        "--variables",
        type=str,
        ...
        help=("Names of variables to use. Options are: {}. If not specified, all are used except massFull").format(
            ", ".join(_varList)
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--looseSIP", action="store_true", help="Use scale factors for SIP<10 with no extra IP cuts.")
    parser.add_argument("--noSIP", action="store_true", help="Use scale factors for no SIP cut and no extra IP cuts.")
    parser.add_argument("--sfRemake", action="store_true", help="Use homebrewed scale factors for electrons.")
    parser.add_argument("--redo", action="store_true", help="Make new histograms even if some are cached.")
    ...
    if not _exists(args.plotDir):
        ...
        raise OSError(f"There is already some non-directory object called {args.plotDir}.")
