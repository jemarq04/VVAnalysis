...
extern struct {
  float pec[SIZE_C][SIZE_E], pce[SIZE_E][SIZE_C], pc[SIZE_C], ne[SIZE_E], nc[SIZE_C], nec_mc[SIZE_C][SIZE_E], nc_mc[SIZE_C], Vc[SIZE_C][SIZE_C], eff[SIZE_C],
      eff_0, eff_true, Vc0[SIZE_C][SIZE_C], Vc1[SIZE_C][SIZE_C], M_unf[SIZE_E][SIZE_C], Vc_u[SIZE_C][SIZE_C], Vc0_u[SIZE_C][SIZE_C], Vpc[SIZE_C][SIZE_C],
...
RooUnfoldDagostini::RooUnfoldDagostini(const RooUnfoldDagostini& rhs) : RooUnfold(rhs) {
  //! Copy constructor.
  Init();
  _niter = rhs._niter;
}

RooUnfoldDagostini::RooUnfoldDagostini(const RooUnfoldResponse* res, const TH1* meas, Int_t niter, const char* name, const char* title)
...
RooUnfoldDagostini* RooUnfoldDagostini::Clone(const char* newname) const {
...
void RooUnfoldDagostini::Init() { GetSettings(); }

void RooUnfoldDagostini::Reset() {
  Init();
  RooUnfold::Reset();
}

void RooUnfoldDagostini::Assign(const RooUnfoldDagostini& rhs) {
  RooUnfold::Assign(rhs);
  _niter = rhs._niter;
}

void RooUnfoldDagostini::Unfold() {
  Int_t nt = _nt, nm = _nm;
  if (nt > SIZE_C) {
  ...
  if (_haveCovMes)
    cerr << "Warning: BAYES does not account for bin-bin correlations on measured input" << endl;
  ...
  if (_res->FakeEntries()) {
    ...
    if (_verbose >= 1)
      cout << "Add truth bin for " << nfakes << " fakes" << endl;
    ntf++;
    ntrue += nfakes;
  ...
  if (ier != 0)
    cerr << "BAYES returned error " << ier << endl;
...
void RooUnfoldDagostini::GetCov() {
...
void RooUnfoldDagostini::GetSettings() {
  _minparm = 1;
  _maxparm = 15;
  _stepsizeparm = 1;
  _defaultparm = 4;
