...
RooUnfold::RooUnfold(const RooUnfoldResponse* res, const TH1* meas, const char* name, const char* title) : TNamed(name, title) {
...
RooUnfold* RooUnfold::New(Algorithm alg, const RooUnfoldResponse* res, const TH1* meas, Double_t regparm, const char* name, const char* title) {
  /*!
...
RooUnfold::RooUnfold(const RooUnfold& rhs) : TNamed(rhs.GetName(), rhs.GetTitle()) {
  //! Copy constructor.
  Init();
  CopyData(rhs);
}
...
void RooUnfold::GetWgt() {
  ...
  if (!_haveCov)
    return;
  if (!InvertMatrix(_cov, _wgt, "covariance matrix", _verbose))
    return;
  _haveWgt = true;
}
...
Bool_t RooUnfold::UnfoldWithErrors(ErrorTreatment withError, bool getWeights) {
  if (!_unfolded) {
    ...
    if (_meas->GetDimension() != rmeas->GetDimension() || _meas->GetNbinsX() != rmeas->GetNbinsX() || _meas->GetNbinsY() != rmeas->GetNbinsY() ||
        _meas->GetNbinsZ() != rmeas->GetNbinsZ()) {
      cerr << "Warning: measured " << _meas->GetNbinsX();
      if (_meas->GetDimension() >= 2)
        cerr << "x" << _meas->GetNbinsY();
      if (_meas->GetDimension() >= 3)
        cerr << "x" << _meas->GetNbinsZ();
      cerr << "-bin histogram does not match " << rmeas->GetNbinsX();
      if (rmeas->GetDimension() >= 2)
        cerr << "x" << rmeas->GetNbinsY();
      if (rmeas->GetDimension() >= 3)
        cerr << "x" << rmeas->GetNbinsZ();
...
void RooUnfold::PrintTable(std::ostream& o, const TH1* hTrue, ErrorTreatment withError) {
  ...
  PrintTable(o, response()->Htruth(), response()->Hmeasured(), hTrue, Hmeasured(), hReco, _nm, _nt, _overflow, withError, chi_squ);
}

void RooUnfold::PrintTable(
    std::ostream& o, const TVectorD& vTrainTrue, const TVectorD& vTrain, const TVectorD& vMeas, const TVectorD& vReco, Int_t nm, Int_t nt) {
  if (nm <= 0)
    nm = vTrain.GetNrows();
  if (nt <= 0)
    nt = vTrainTrue.GetNrows();
...
void RooUnfold::PrintTable(std::ostream& o,
                           const TH1* hTrainTrue,
                           const TH1* hTrain,
                           const TH1* hTrue,
                           const TH1* hMeas,
                           const TH1* hReco,
  ...
  //! Prints entries from truth, measured, and reconstructed data for each bin.
  if (withError == kDefault)
    withError = hReco->GetSumw2N() ? kErrors : kNoError;
  if (_nm <= 0)
    _nm = hTrain->GetNbinsX();
  if (_nt <= 0)
    _nt = hTrainTrue->GetNbinsX();
  std::ostringstream fmt;
  fmt.copyfmt(o);
  Int_t dim = hReco->GetDimension(), ntxb = hReco->GetNbinsX() + 2, ntyb = hReco->GetNbinsY() + 2;
  if (hMeas->GetDimension() != dim || hMeas->GetNbinsX() + 2 != ntxb || hMeas->GetNbinsY() + 2 != ntyb)
    dim = 1;
  ...
  for (Int_t i = 0; i < maxbin; i++) {
    ...
    if (i < _nt) {
      ...
      if (withError)
        o << ' ' << setw(9) << yerr;
      if (hTrue && (y != 0.0 || (withError && yerr > 0.0)) && (hTrue->GetBinContent(it) != 0.0 || (withError && hTrue->GetBinError(it) > 0.0))) {
  ...
  o << "===============================================================================" << xwid << endl
    << setw(iwid) << "" << std::fixed << setprecision(0) << ' ' << setw(8) << true_train_tot << ' ' << setw(8) << meas_train_tot;
  if (hTrue)
  ...
  o << ' ' << setw(8) << meas_test_tot << setprecision(1) << ' ' << setw(8) << unf_tot;
  if (withError)
    o << ' ' << setw(9) << toterr;
...
RooUnfold* RooUnfold::RunToy() const {
  ...
  if (_dosys)
    unfold->SetResponse(_res->RunToy(), kTRUE);
  if (_dosys == 2)
    return unfold;
  ...
  if (_haveCovMes) {
    ...
    if (!_covL) {
      ...
      if (_verbose >= 2)
        RooUnfoldResponse::PrintMatrix(*_covL, "decomposed measurement covariance matrix");
    }
...
void RooUnfold::Print(Option_t* /*opt*/) const {
  cout << ClassName() << "::" << GetName() << " \"" << GetTitle() << "\", regularisation parameter=" << GetRegParm() << ", ";
  if (_haveCovMes)
  ...
  if (_meas->GetDimension() == 1)
    ...
    cout << _meas->GetNbinsX() << "x" << _meas->GetNbinsY();
    if (_meas->GetDimension() >= 3)
  ...
  if (rtrue->GetDimension() == 1)
    cout << _nt;
  else {
    cout << rtrue->GetNbinsX() << "x" << rtrue->GetNbinsY();
...
Int_t RooUnfold::InvertMatrix(const TMatrixD& mat, TMatrixD& inv, const char* name, Int_t verbose) {
  ...
  if (verbose >= 1) {
    ...
    if (verbose >= 3)
      RooUnfoldResponse::PrintMatrix(I, "V*V^-1");
    Double_t m = 0.0;
