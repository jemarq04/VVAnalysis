...
for file_name in ["WZxsec2016_aQGC-" + i for i in ["FT", "FM", "FS"]]:
    with open(ConfigureJobs.getManagerPath() + "/AnalysisDatasetManager/PlotGroups/%s.json" % file_name) as plot_file:
...
aqgc_hists = HistTools.make1DaQGCHists(
    input_file,
    ...
    ["MTWZ_lheWeights_%s_%s" % (var, c) for c in ConfigureJobs.getChannels() for var in variations]
