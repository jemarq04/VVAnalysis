...
protected:
  ...
  Double_t getChi2(const TVectorD& prob1, const TVectorD& prob2, Double_t nevents) const;
...
inline RooUnfoldBayes::RooUnfoldBayes() : RooUnfold() {
  // Default constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfoldBayes::RooUnfoldBayes(const char* name, const char* title) : RooUnfold(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfoldBayes::RooUnfoldBayes(const TString& name, const TString& title) : RooUnfold(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}
...
inline void RooUnfoldBayes::SetIterations(Int_t niter) {
  // Set regularisation parameter (number of iterations)
  _niter = niter;
}

inline void RooUnfoldBayes::SetSmoothing(Bool_t smoothit) {
  // Enable smoothing
  _smoothit = smoothit;
}

inline Int_t RooUnfoldBayes::GetIterations() const {
  // Return regularisation parameter (number of iterations)
  return _niter;
}

inline Int_t RooUnfoldBayes::GetSmoothing() const {
...
inline void RooUnfoldBayes::SetRegParm(Double_t parm) {
  // Set regularisation parameter (number of iterations)
  SetIterations(Int_t(parm + 0.5));
}

inline Double_t RooUnfoldBayes::GetRegParm() const {
  // Return regularisation parameter (number of iterations)
