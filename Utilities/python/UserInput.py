...
def getDefaultParser():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of selection to make,  as defined in Cuts/<analysis>/<selection>.json",
    )
    parser.add_argument(
        "-v", "--version", type=str, required=False, default="1", help="Version number, appended to name"
    )
    parser.add_argument(
        "-a",
        "--analysis",
    ...
    input_group.add_argument(
        ...
        help="List of input file names, as defined in ZZ4lDatasetManager, separated by commas",
    )
    input_group.add_argument(
        "--inputs_from_file",
        nargs=3,
        metavar=("filelist", "nPerJob", "jobNum"),
        help="Text file with one input file per line, number of files to process per job, job number",
    )
    return parser
...
# Depends on ZZ4lDatasetManagerModule
def getHistInfo(analysis, input_hists, noConfig=False):
    ...
    ConfigHistTools = imp.load_source(
        "ConfigHistTools", "/".join([manager_path, "ZZ4lDatasetManager/Utilities/python/ConfigHistTools.py"])
    )
