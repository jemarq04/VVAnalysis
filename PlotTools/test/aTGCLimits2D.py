...
from PlotTools import PlotStyle, makeLegend, pdfViaTex
...
for nf, fName in fNames.items():
    ...
    bars1D = Graph(1, type="asymm", title=r"\textbf{Obs.\ 95\% CL (1D)}", color="r", drawstyle="ZL", legendstyle="L")
    ...
    leg = makeLegend(c, *graphs.values(), leftmargin=0.023, rightmargin=0.452, topmargin=0.63, textsize=0.036)
    ...
    xaxis.SetTitle(rf"f_{nf}^\gamma")
    yaxis.SetLimits(*axlimits)
    yaxis.SetRangeUser(*axlimits)
    yaxis.SetTitle(rf"f_{nf}^\text{{Z}}")  # r'\mathbf{{f}}_\mathbf{{{}}}^\textbf{{Z}}'.format(nf))
    ...
    c.Print(pjoin(outdir, f"limits2D_f{nf}.png"))
    style.setCMSStyle(c, "", True, "", intLumi=35860.0, forLatex=True)

    pdfViaTex(c, f"limits2D_f{nf}", texdir, pdfdir)
