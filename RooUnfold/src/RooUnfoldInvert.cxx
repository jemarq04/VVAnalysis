...
RooUnfoldInvert::RooUnfoldInvert(const RooUnfoldInvert& rhs) : RooUnfold(rhs) {
  //! Copy constructor.
  Init();
}

RooUnfoldInvert::RooUnfoldInvert(const RooUnfoldResponse* res, const TH1* meas, const char* name, const char* title) : RooUnfold(res, meas, name, title) {
  //! Constructor with response matrix object and measured unfolding input histogram.
  Init();
}

RooUnfoldInvert* RooUnfoldInvert::Clone(const char* newname) const {
...
void RooUnfoldInvert::Init() {
  _svd = 0;
  _resinv = 0;
  GetSettings();
}

void RooUnfoldInvert::Reset() {
  delete _svd;
  delete _resinv;
  Init();
  RooUnfold::Reset();
...
TDecompSVD* RooUnfoldInvert::Impl() { return _svd; }

void RooUnfoldInvert::Unfold() {
  if (_nt > _nm) {
...
Bool_t RooUnfoldInvert::InvertResponse() {
  if (!_svd)
    return false;
  if (_resinv)
    return true;
  if (_nt > _nm)
    _resinv = new TMatrixD(_nm, _nt);
  else
    _resinv = new TMatrixD(_nt, _nm);
...
void RooUnfoldInvert::GetSettings() {
  _minparm = 0;
  _maxparm = 0;
  _stepsizeparm = 0;
  _defaultparm = 0;
