...
def combineYears(l16, l17, l18, w16, w17, w18):
    ...
    luncorr = [
        math.copysign(1.0, z) * ((w16 * x) ** 2 + (w17 * y) ** 2 + (w18 * z) ** 2) ** 0.5
        for (x, y, z) in zip(l16, l17, l18)
    ]
...
if printTable:
        ...
        if n == 0:
            ...
            for k, nsys in enumerate(fn_sys):
                sysdict[nsys] = [
                    ...
                    fn_uncorr[k],
                    fn_uncorr[k],
                ]  # len 4 list of uncorr and corr best min/max values
        else:
            for k, nsys in enumerate(fn_sys):
    ...
    for key in keysReorder:
        if "jer" in key:
            ...
            erange = sysdict[key][:2]
        print(f"& {100 * round(min(erange), 4)} - {100 * round(max(erange), 4)} \\" + "%")
        ...
        for key in keysReorder:
            ...
            for n, var in enumerate(varPi.split(" ")):
                ...
                for k, nsys in enumerate(fn_sys):
                    if key == nsys:
                        erange2 = fn_corr[k]
                pstr += f"& {100 * round(erange2, 4)} "
                if round(erange2, 4) > 0.0:
                    pstr += "\\%"
                if n < len(varPi) - 1:
                    pstr += " "
            print(pstr)
