...
from PlotTools import makeErrorBand, makeLegend
from PlotTools import pdfViaTex as _pdfViaTex
from Utilities import deltaPhiString, deltaRString, makeNumberPretty
...
_binning4l = {
    "Mass": [100.0] + [200.0 + 50.0 * i for i in range(5)] + [500.0, 600.0, 800.0, 1000.0, 1200.0],
    "Pt": [0.0, 5.0]
    + [25.0 + 25.0 * i for i in range(3)]
    + [100.0, 150.0, 200.0, 300.0],  # [25.*i for i in range(4)] + [100., 150., 200., 300.],
    "Eta": [16, -5.0, 5.0],
    #'Phi'   : [12, -3.15, 3.15],
    "nvtx": [40, 0.0, 40.0],
    "nJets": [5, -0.5, 4.5],
    "nJets_eta2p4": [6, -0.5, 5.5],
    "jet1Pt": [30.0, 50.0, 100.0, 200.0, 300.0, 500.0],
    "jet1Eta": [0.0, 1.5, 2.4, 3.2, 4.7],
    "jet2Pt": [30.0, 50.0, 100.0, 170.0, 300.0],
    "jet2Eta": [0.0, 1.5, 3.0, 4.7],
    "mjj": [0.0, 200.0, 400.0, 600.0, 1000.0],
    "deltaEtajj": [4, 0.0, 4.7],
...
_vars4l = {v: dict.fromkeys(["eeee", "eemm", "mmmm"], _vars4l[v]) for v in _vars4l}
_vars4l["deltaPhiZZ"] = {
    "eeee": f"{deltaPhiString()}(e1_e2_Phi, e3_e4_Phi)",
    "eemm": f"{deltaPhiString()}(e1_e2_Phi, m1_m2_Phi)",
    "mmmm": f"{deltaPhiString()}(m1_m2_Phi, m3_m4_Phi)",
}
_vars4l["deltaRZZ"] = {
...
_selections4l = dict.fromkeys(_vars4l, "")
...
_binning2l = {
    "Mass": [60, 60.0, 120.0],
    "Pt": [i * 25.0 for i in range(7)] + [200.0, 300.0],
    "Eta": [48, -6.0, 6.0],
    "Phi": [24, -3.15, 3.15],
}
...
_selections2l = dict.fromkeys(_varTemplates2l, "")
_selections2l["z1"] = {
    "eeee": "",
    "mmmm": "",
    "eemm": [
...
def _makeSystematics(
    ...
    hBkgNom = bkg.makeHist(var, sel, binning, postprocess=True, perUnitWidth=norm)
    ...
    for sys in "up", "dn":
        ...
        hSigSyst["pu"][sys] = sig.makeHist(var, sel, binning, perUnitWidth=norm)
        hIrrSyst["pu"][sys] = irr.makeHist(var, sel, binning, perUnitWidth=norm)
    ...
    for lep in set("".join(var.keys())):
        ...
        for sys in ["up", "dn"]:
            wtArg = {lep + "Syst": sys}
            wtArg.update(sfArgs)
            wtStr = baseMCWeight("zz", puWeightFile, **wtArg)
            sig.applyWeight(wtStr, True)
            irr.applyWeight(wtStr, True)
            hSigSyst[lep + "Syst"][sys] = sig.makeHist(var, sel, binning, perUnitWidth=norm)
            hIrrSyst[lep + "Syst"][sys] = irr.makeHist(var, sel, binning, perUnitWidth=norm)
    ...
    for lep in relevantLeptons:
        ...
        for sys in ["up", "dn"]:
            hBkgSyst[lep + "FR"][sys] = bkgSyst[lep + sys].makeHist(
                relevantVar, sel, binning, perUnitWidth=norm, postprocess=True
            )
            if irrelevantVar:
                hBkgSyst[lep + "FR"][sys] += bkg.makeHist(
                    irrelevantVar, sel, binning, perUnitWidth=norm, postprocess=True
                )
    ...
    if eVar:
        for sys in ["eScale", "eRhoRes", "ePhiRes"]:
            ...
            for shift in ["up", "dn"]:
                ...
                hSigSyst[sys][shift] = sigSyst[sys + sysStr].makeHist(eVar, sel, binning, perUnitWidth=norm)
                hIrrSyst[sys][shift] = irrSyst[sys + sysStr].makeHist(eVar, sel, binning, perUnitWidth=norm)
                if noEVar:
                    hSigSyst[sys][shift] += sig.makeHist(noEVar, sel, binning, perUnitWidth=norm)
                    hIrrSyst[sys][shift] += irr.makeHist(noEVar, sel, binning, perUnitWidth=norm)
    ...
    if muVar:
        ...
        for shift in ["up", "dn"]:
            ...
            hSigSyst[sys][shift] = sigSyst[sys + sysStr].makeHist(muVar, sel, binning, perUnitWidth=norm)
            hIrrSyst[sys][shift] = irrSyst[sys + sysStr].makeHist(muVar, sel, binning, perUnitWidth=norm)
            if noMuVar:
                hSigSyst[sys][shift] += sig.makeHist(noMuVar, sel, binning, perUnitWidth=norm)
                hIrrSyst[sys][shift] += irr.makeHist(noMuVar, sel, binning, perUnitWidth=norm)
    ...
    for c, v in var.iteritems():
        ...
        for s in sig[c].values():
            if "GluGluZZ" in s.name:
                ...
                hThisEWK = s.makeHist(v, thisSel, binning, perUnitWidth=norm)
                if hEWK is None:
                    hEWK = hThisEWK
                else:
                    hEWK += hThisEWK
                ...
                hSigVariations.append(
                    s.makeHist2(
                        v, "Iteration$", thisSel, binning, [100, 0.0, 100.0], "pdfWeights / pdfWeights[0]", False
                    )
                )
                hThisQQ = s.makeHist(v, thisSel, binning, perUnitWidth=norm)
                if "up" not in hSigSyst["pdf"]:
        ...
        for c, v in var.iteritems():
            ...
            for s in sig[c].values():
                if "GluGluZZ" not in s.name and "phantom" not in s.name:
                    hSigVars[-1] += s.makeHist(
                        v, thisSel, binning, f"scaleWeights[{ind}] / scaleWeights[0]", perUnitWidth=norm
                    )
        hIrrVars.append(irr.makeHist(var, sel, binning, f"scaleWeights[{ind}] / scaleWeights[0]", perUnitWidth=norm))
    ...
    for ind in alphaSIndices:
        ...
        for c, v in var.iteritems():
            ...
            for s in sig[c].values():
                if "GluGluZZ" not in s.name and "phantom" not in s.name:
                    hSigVars[-1] += s.makeHist(v, thisSel, binning, f"pdfWeights[{ind}]", perUnitWidth=norm)
    ...
    if ("jet" in varName.lower() or "jj" in varName.lower()) and "eta2p4" not in varName.lower():
        for sys in ["jer", "jes"]:
            ...
            for shift in ["up", "dn"]:
                ...
                hSigSyst[sys][shift] = sig.makeHist(varShifted, selShifted, binning, perUnitWidth=norm)
                hIrrSyst[sys][shift] = irr.makeHist(varShifted, selShifted, binning, perUnitWidth=norm)
    ...
    for sys in set(hSigSyst.keys() + hBkgSyst.keys() + hIrrSyst.keys()):
        ...
        thisDn = (
            hSigSyst.get(sys, {}).get("dn", hSigNom)
            - hSigNom
...
def main(
    ...
    if doSyst:
        ...
        irr = zzIrreducibleBkg("zz", inMC, ana, puWeightFile, lumi, **sfArgs)

        sigSyst = {}
        irrSyst = {}
        for syst in ["eScaleUp", "eScaleDn", "eRhoResUp", "eRhoResDn", "ePhiResUp"]:
            sigSyst[syst] = zzStackSignalOnly(
                "eeee,eemm",
            ...
            irrSyst[syst] = zzIrreducibleBkg(
                "eeee,eemm", inMC.replace("mc_", f"mc_{syst}_"), ana, puWeightFile, lumi, **sfArgs
            )
        ...
        for syst in ["mClosureUp", "mClosureDn"]:
            ...
            irrSyst[syst] = zzIrreducibleBkg(
                "eemm,mmmm", inMC.replace("mc_", f"mc_{syst}_"), ana, puWeightFile, lumi, **sfArgs
            )

        bkg = standardZZBkg(
            "zz", inData, inMC, ana, puWeightFile, fakeRateFile, lumi, eras=eras, sipCut=sipForBkg, **sfArgs
        )
        bkgSyst = {
            "eup": standardZZBkg(
                "zz",
    ...
    if ana == "smp":
        ...
        aTGCf4 = standardZZMC("zz", aTGCDataset, "ZZTo4L-aTGC-f4-fg0p0019-fz0p0015", ana, puWeightFile, lumi)
        aTGCf5 = standardZZMC("zz", aTGCDataset, "ZZTo4L-aTGC-f5-fg0p0019-fz0p0015", ana, puWeightFile, lumi)
        sherpa = standardZZMC("zz", aTGCDataset, "ZZTo4L-sherpa", ana, puWeightFile, lumi)
    ...
    if ana == "z4l":
        binning4l["Mass"] = [20, 80.0, 100.0]
        binNormWidth4l["Mass"] = 1.0
    elif ana == "full":
        binning4l["Mass"] = [25.0 * i for i in range(17)] + [500.0, 600.0, 800.0]
        binNormWidth4l["Mass"] = 25.0
    elif ana == "smp":
        binning4l["Mass"] = [b / 1000.0 for b in binning4l["Mass"]]
        binNormWidth4l["Mass"] /= 1000.0
    ...
    for chan in ["zz", "eeee", "eemm", "mmmm"]:
        ...
        for varName, binning in binning4l.iteritems():
            ...
            hStack = stack.makeHist(var, sel, binning, postprocess=True, perUnitWidth=norm)
            dataPts = data.makeHist(var, dataSelection, binning, poissonErrors=True, perUnitWidth=norm)
            if paper:
                dataPts.title = rf"\textbf{{{dataPts.title}}}"
            toPlot = [hStack, dataPts]

            if doSyst:
                hSystUp, hSystDn = _makeSystematics(
                    varName, var, sel, binning, sig, bkg, irr, sigSyst, bkgSyst, irrSyst, norm, puWeightFile, sfArgs
                )
            ...
            yTitle = "Events"
            if norm:
                yTitle += f" / {makeNumberPretty(normForYAxis, 2)} {units[varName]}"

            if varName == "Mass" and ana == "smp":
                xTitle = xTitle.replace("GeV", "TeV")
                if norm:
                    yTitle = yTitle.replace(
                        makeNumberPretty(normForYAxis, 2), makeNumberPretty(normForYAxis / 1000.0, 4)
                    ).replace("GeV", "TeV")
            ...
            if varName == "Mass" and ana == "smp" and chan == "zz":
                hTGCf4 = aTGCf4.makeHist(var, _selections4l[varName], binning, perUnitWidth=norm, mergeOverflow=True)
                hTGCf5 = aTGCf5.makeHist(var, _selections4l[varName], binning, perUnitWidth=norm, mergeOverflow=True)
                hSherpa = sherpa.makeHist(var, _selections4l[varName], binning, perUnitWidth=norm, mergeOverflow=True)

                for s in stack:
                    if s.name not in ["ZZTo4L", "ZZTo4L-amcatnlo"]:
                        hToAdd = s.makeHist(
                            var,
                            _selections4l[varName],
                            binning,
                            postprocess=True,
                            perUnitWidth=norm,
                            mergeOverflow=True,
                ...
                style.setCMSStyle(cTGC, "", dataType=typeToPrint, intLumi=lumi, forLatex=paper)
                if paper:
                    _pdfViaTex(cTGC, chan + varName + "_aTGC", texDir, pdfDir)
                else:
                    cTGC.Print(f"{outdir}/{chan}{varName}_aTGC.png")

    binning2l = _binning2l.copy()
    binNormWidth2l = _binNormWidth2l.copy()

    if ana != "smp":
        binning2l["Mass"] = [60, 0.0, 120.0]
    ...
    for z in ["z", "ze", "zm", "z1", "z2"]:
        for varName, binning in binning2l.iteritems():
            ...
            print(f"Plotting {z} {varName}")

            var = {c: [vt.format(var=varName) for vt in _varTemplates2l[z][c]] for c in _varTemplates2l[z]}

            hStack = stack.makeHist(
                var, _selections2l[z], binning, postprocess=True, perUnitWidth=binNormWidth2l[varName]
            )
            dataPts = data.makeHist(
                var, _selections2l[z], binning, poissonErrors=True, perUnitWidth=binNormWidth2l[varName]
            )

            if paper:
                dataPts.title = rf"\textbf{{{dataPts.title}}}"

            if varName == "Pt" and binning2l["Pt"][-1] > 200.0:
                copy = dataPts.clone()
                x = copy.GetX()
            ...
            yTitle = f"Z bosons / {makeNumberPretty(binNormWidth2l[varName], 2)} {units[varName]}"
            ...
            if blind and varName == "Pt" and binning2l["Pt"][-1] > 200.0:
                box = TBox(max(xmin, 200.0), ymin, min(binning2l["Pt"][-1], xmax), ymax)
                box.SetFillColor(1)
                box.SetFillStyle(3002)
                box.Draw("same")
                leg.SetFillStyle(1001)
    ...
    for varName, binning in _binning1l.iteritems():
        ...
        for lep in _vars1l[varName]:
            print(f"Plotting {lep} {varName}")

            var = _vars1l[varName][lep]

            hStack = stack.makeHist(
                var, _selections1l[lep], _binning1l[varName], postprocess=True, perUnitWidth=_binNormWidth1l[varName]
            )
            dataPts = data.makeHist(
                var, _selections1l[lep], _binning1l[varName], poissonErrors=True, perUnitWidth=_binNormWidth1l[varName]
            )
            if paper:
                dataPts.title = rf"\textbf{{{dataPts.title}}}"
            ...
            if paper:
                ...
                c.Print(f"{outdir}/{lep}{varName}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataDir", type=str, nargs="?", default="uwvvNtuples_data_20feb2017", help="Directory where data ntuples live"
    )
    parser.add_argument(
        "--mcDir", type=str, nargs="?", default="uwvvNtuples_mc_20feb2017", help="Directory where MC ntuples live"
    )
    parser.add_argument(
        "--plotDir",
        type=str,
        nargs="?",
        default="zzPlots",
        help="Directory to put plots in, possibly relative to ~/www/UWVVPlots",
    )
    parser.add_argument(
        "--fakeRateFile",
        type=str,
    ...
    parser.add_argument(
        "--lumi", type=float, nargs="?", default=35860.0, help="Integrated luminosity of sample (in pb^-1)"
    )
    parser.add_argument(
        "--amcatnlo", action="store_true", help="Use MadGraph5_aMC@NLO as the primary MC instead of Powheg."
    )
    parser.add_argument("--eras", type=str, nargs="?", default="BCDEFGH", help="Data eras to use.")
    parser.add_argument(
        "--analysis", "--ana", type=str, nargs="?", default="smp", help="Which set of cuts to use (full, smp, etc.)."
    )
    parser.add_argument("--blind", action="store_true", help="Put blinding boxes on a few distributions.")
    parser.add_argument("--noSyst", action="store_true", help="Don't make a hatched band for systematic errors.")
    parser.add_argument("--logy", "--logY", "--log", action="store_true", help="Put the vertical axis on a log scale")
    ...
    parser.add_argument(
        "--paper", action="store_true", help="Print as journal-style PDFs, only the plots for the paper."
    )
