...
nocorr_file = ROOT.TFile(
    "/afs/cern.ch/user/k/kelong/public/SMP-18-001/Higgs/NoCorrection/VBSselection_Loose_Full-04Oct2018_Higgs_NoPrefireCorrection_wCR.root"
)
corr_file = ROOT.TFile(
    "/afs/cern.ch/user/k/kelong/public/SMP-18-001/Higgs/WithPrefiringCorrection/VBSselection_Loose_Full-04Oct2018_Higgs_WithPrefireCorrection_wCR.root"
)
...
names = {"EWWZ": "EW-WZjj", "WZ": "QCD-WZjj", "VVV": "top-ewk", "ZZ": "vv", "Fake": "DataEWKCorrected", "Zg": "zg"}
...
for folder in list(names.values()):
    ...
    cr_err = cr_ratio * math.sqrt(
        (corr_hist.GetBinError(1) / corr_hist.GetBinContent(1)) ** 2
        + (uncorr_hist.GetBinError(1) / uncorr_hist.GetBinContent(1)) ** 2
    )
    print(
        "Overall correction to yield for sample %s is %0.3f in CR and %0.3f in signal" % (folder, cr_ratio, int_ratio)
    )
...
for hist_key in jakobs_file.GetListOfKeys():
    ...
    if not correction_hist.GetNbinsX() == hist.GetNbinsX():
        raise RuntimeError(
            "Unequal number of bins for correction and central hist"
