...
parser.add_argument("-c", "--channels", required=False, type=str, default="eeee,eemm,mmmm")
parser.add_argument("-o", "--output_dir", required=False, type=str, default="/eos/user/u/uhussain/ZZDuplicates")
args = parser.parse_args()
path = "/cms/kdlong" if "hep.wisc.edu" in os.environ["HOSTNAME"] else "/afs/cern.ch/user/u/uhussain/work"
isfile = any(os.path.isfile(name) or os.path.exists(name.rstrip("/*")) for name in args.filelist)
filelist = ConfigureJobs.getListOfFiles(args.filelist, path) if not isfile else args.filelist
...
if args.printEventNums:
    output_dir = "/".join(
        [args.output_dir, "EventYields_Usama_{:%Y-%m-%d}".format(datetime.date.today()), args.output_selection]
    )
...
for name in filelist:
    if not isfile:
        try:
            file_path = ConfigureJobs.getInputFilesPath(name, path, args.selection, "ZZ4l2022")
...
event_info.add_row(
    ["All files", totals["eeee"], totals["eemm"], totals["mmmm"], sum(list(totals.values())[:-1]), totals["processed"]]
)
