    ...
    for i in chain:
        if deduplicate:
            if "DoubleMuon" in datasets:
                ...
                if not (
                    (i.singleMuPass or i.singleIsoMuPass)
                    and not (i.doubleMuPass or i.singleESingleMuPass or i.singleMuSingleEPass)
                ):
                    continue
            else:
                if not (
                    i.singleESingleMuPass
                    or i.singleMuSingleEPass
                    or i.doubleMuPass
                    or i.singleMuPass
                    or i.singleIsoMuPass
                ):
                    continue
        events.append(f"{i.run}:{i.lumi}:{i.evt}")
        event_map.update(
            {
                f"{i.run}:{i.lumi}:{i.evt}": i.GetFile().GetName()
...
with open("details.txt", "w") as eventfile:
    eventfile.writelines(f"{i} -- {event_map[i]}\n" for i in missingevents)
