...
void ScaleFactor::RegisterGlobalFunction(int dimension) {
  ...
  if (dimension == 1) {
    TString fDecl(Form(
        "double %s(double x) { auto ptr = (ScaleFactor*) gROOT->GetListOfSpecials()->FindObject(\"%s\"); return ptr->Evaluate1D(x); }", GetName(), GetName()));
    interpreter->Declare(fDecl);
  } else if (dimension == 2) {
    TString fDecl(
        Form("double %s(double x, double y) { auto ptr = (ScaleFactor*) gROOT->GetListOfSpecials()->FindObject(\"%s\"); return ptr->Evaluate2D(x, y); }",
             GetName(),
             GetName()));
    interpreter->Declare(fDecl);
  } else {
