...
_binning = {
    ...
    "mass": [100.0] + [200.0 + 50.0 * i for i in range(5)] + [500.0, 600.0, 800.0, 2000.0],
...
def unfold(
    ...
    tmpFileName = "PowResponseMatrix-%s-%s_%s.root" % (today, analysis, varName)
...
# today = datetime.date.today().strftime("%d%b%Y")
# tmpFileName = "PowMatrix_MGUnfHistsExtraBins-%s-%s.root" % (today, analysis)
# fHistOut = ROOT.TFile.Open(tmpFileName, "update")
##fOut = ROOT.TFile.Open("/".join([outputFolder, outputFile]), "RECREATE")
# fHistOut.cd()
