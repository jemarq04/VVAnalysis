...
for state in ["eee", "eem", "emm", "mmm"]:
    chain = ROOT.TChain(f"{state}/ntuple")
    chain.Add(
        "/data/kelong/DibosonAnalysisData/Preselection/2016-11-02-wz3lnu-powheg-WZxsec2016-preselection-v1/skim-ntuplize_cfg-086609CA-D81B-E611-BF6C-002590D9D896.root"
    )
    selector = ROOT.TSelector.GetSelector("../../Utilities/Selectors/disambiguateFinalStates.C+")
    print("-" * 80)
    print("channel is", state)
    print("Initial number of entries is", chain.GetEntries())
    print("Events with list", chain.Draw("Mass", ""))
    ...
    print("Events in the list", entryList.GetN())
    print("Events with list", new_chain.Draw("Mass", ""))
    del entryList
