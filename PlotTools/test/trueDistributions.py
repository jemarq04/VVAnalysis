...
from PlotTools import makeErrorBand, makeLegend, pdfViaTex
from Utilities import combineWeights, deltaPhiString, deltaRString
...
_variables = {
    ...
    "eta": dict.fromkeys(channels, "abs(Eta)"),
...
_selections = {
    "pt": dict.fromkeys(channels, ""),
    "mass": dict.fromkeys(channels, ""),
    "eta": dict.fromkeys(channels, ""),
    "zPt": dict.fromkeys(channels, ""),
    "zHigherPt": {
    ...
    "deltaPhiZZ": dict.fromkeys(channels, ""),
    "deltaRZZ": dict.fromkeys(channels, ""),
    "l1Pt": dict.fromkeys(channels, ""),
}
...
_legParams["eta"]["leftmargin"] = 0.5
_legParams["eta"]["rightmargin"] = 0.03
_legParams["eta"]["topmargin"] = 0.05
...
def _scaleErrors(sample, var, sel, binning, hNominal):
    ...
    for bUp, bDn, bNom, variations in zip(outUp, outDn, hNominal, zip(*hTrues)):
...
if not (_exists(_join(plotDir, "texs")) and _exists(_join(plotDir, "pdfs"))):
    ...
    raise OSError("There is already some non-directory object called {}.".format(_join(plotDir, "texs")))
elif not _isdir(_join(plotDir, "pdfs")):
    raise OSError("There is already some non-directory object called {}.".format(_join(plotDir, "pdfs")))

ana = "smp"


sPow = genZZSamples("zz", inMC, ana, 1000.0)
sMG5 = genZZSamples("zz", inMC, ana, 1000.0, amcatnlo=True)
...
for varName in _variables:
    ...
    hNominalPow = {c: sPow[c].makeHist(var[c], sel[c], bins, perUnitWidth=False) for c in channels}
    hNominalPow[""] = sPow.makeHist(var, sel, bins, perUnitWidth=False)
    hNominalMG5 = {c: sMG5[c].makeHist(var[c], sel[c], bins, perUnitWidth=False) for c in channels}
    hNominalMG5[""] = sMG5.makeHist(var, sel, bins, perUnitWidth=False)

    hPDFErrPow = {c: _pdfError(sPow[c], var[c], sel[c], bins) for c in channels}
    hPDFErrPow[""] = sum(hPDFErrPow.values())
    hPDFErrMG5 = {c: _pdfError(sMG5[c], var[c], sel[c], bins) for c in channels}
    ...
    for c in channels:
        hScaleUpPow[c], hScaleDnPow[c] = _scaleErrors(sPow[c], var[c], sel[c], bins, hNominalPow[c])
        hScaleUpMG5[c], hScaleDnMG5[c] = _scaleErrors(sMG5[c], var[c], sel[c], bins, hNominalMG5[c])
    ...
    for c in hNominalPow:
        ...
        hMG5.legendstyle = "L"
        hMG5.title = f"{mg5Name}"
        hMG5.SetLineWidth(hMG5.GetLineWidth() * 2)

        # uncertainty
        hUncUpPow = hPDFErrPow[c].empty_clone()
        hUncDnPow = hPDFErrPow[c].empty_clone()
        for bUp, bDn, bPDF, bScaleUp, bScaleDn in zip(
            hUncUpPow, hUncDnPow, hPDFErrPow[c], hScaleUpPow[c], hScaleDnPow[c]
        ):
        ...
        for bUp, bDn, bPDF, bScaleUp, bScaleDn in zip(
            hUncUpMG5, hUncDnMG5, hPDFErrMG5[c], hScaleUpMG5[c], hScaleDnMG5[c]
        ):
            bUp.value = sqrt(bPDF.value**2 + bScaleUp.value**2)
            bDn.value = sqrt(bPDF.value**2 + bScaleDn.value**2)
        ...
        _style.setCMSStyle(can, "", dataType="Preliminary Simulation", intLumi=-1)
        outWords = ["true", varName, c]
        pdfViaTex(can, "_".join(w for w in outWords if w), _join(plotDir, "texs"), _join(plotDir, "pdfs"))
