...
def getManagerPath():
    ...
    if "dataset_manager_path" not in config["Setup"]:
        raise ValueError("dataset_manager_path not specified in config file Template/config.%s" % os.environ["USER"])
...
def getListOfHZZFilenames():
    return ["ggHZZ", "vbfHZZ", "ttH_HToZZ_4L", "WminusHToZZ", "WplusHToZZ", "ZHToZZ_4L"]


def getListOfggZZFilenames():
    return ["ggZZ4e", "ggZZ4m", "ggZZ4t", "ggZZ2e2mu", "ggZZ2e2tau", "ggZZ2mu2tau"]
...
def getPreviousStep(selection, analysis):
    ...
    if analysis == "ZZ4l2016":
        selection_map = {"ntuples": "ntuples", "preselection": "ntuples", "4lCRBase": "ntuples"}
