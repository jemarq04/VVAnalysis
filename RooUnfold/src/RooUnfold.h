...
public:
  ...
  static RooUnfold* New(Algorithm alg, const RooUnfoldResponse* res, const TH1* meas, Double_t regparm = -1e30, const char* name = 0, const char* title = 0);
  ...
  static void PrintTable(std::ostream& o,
                         ...
                         Int_t _nm = 0,
                         Int_t _nt = 0,
                         Bool_t _overflow = kFALSE,
                         ErrorTreatment withError = kDefault,
                         Double_t chi_squ = -999.0);

  static void PrintTable(std::ostream& o, const TVectorD& vTrainTrue, const TVectorD& vTrain, const TVectorD& vMeas, const TVectorD& vReco, Int_t nm, Int_t nt);
...
public:
  ClassDef(RooUnfold,
           2)  // Unfolding base class: implementations in RooUnfoldBayes, RooUnfoldSvd, RooUnfoldBinByBin, RooUnfoldTUnfold, RooUnfoldInvert, RooUnfoldIds
};
...
inline RooUnfold::RooUnfold() : TNamed() {
  // Default constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfold::RooUnfold(const char* name, const char* title) : TNamed(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}

inline RooUnfold::RooUnfold(const TString& name, const TString& title) : TNamed(name, title) {
  // Basic named constructor. Use Setup() to prepare for unfolding.
  Init();
}
...
inline RooUnfold& RooUnfold::operator=(const RooUnfold& rhs) {
  // Assignment operator for copying RooUnfold settings.
  Assign(rhs);
  return *this;
}

inline Int_t RooUnfold::verbose() const {
  // Get verbosity setting which controls amount of information to be printed
  return _verbose;
}

inline Int_t RooUnfold::NToys() const {
  // Get number of toys used in kCovToy error calculation.
  return _NToys;
}

inline Int_t RooUnfold::Overflow() const {
  // Histogram under/overflow bins are used?
  return _overflow;
}

inline const RooUnfoldResponse* RooUnfold::response() const {
  // Response matrix object
  return _res;
}

inline const TH1* RooUnfold::Hmeasured() const {
  // Measured Distribution as a histogram
  return _meas;
}

inline TVectorD& RooUnfold::Vreco() {
  ...
  if (!_unfolded) {
    ...
    if (!_unfolded) {
      _fail = true;
      if (_nt > 0 && _rec.GetNrows() == 0)
        _rec.ResizeTo(_nt);  // need something
    }
  }
  return _rec;
}

inline const TVectorD& RooUnfold::Vmeasured() const {
...
inline const TVectorD& RooUnfold::Emeasured() const {
...
inline void RooUnfold::SetVerbose(Int_t level) {
  // Set verbosity level which controls amount of information to be printed
  _verbose = level;
}

inline void RooUnfold::SetNToys(Int_t toys) {
  // Set number of toys used in kCovToy error calculation.
  _NToys = toys;
}

inline void RooUnfold::SetRegParm(Double_t) {
  // Set Regularisation parameter
}

inline Double_t RooUnfold::GetRegParm() const {
  // Get regularisation parameter.
  return -1;
}

inline void RooUnfold::IncludeSystematics(Int_t dosys) {
  // Include systematic errors from response matrix?
  // Use dosys=2 to exclude measurement errors.
  if (dosys != _dosys)
    _haveWgt = _haveErrors = _haveCov = _have_err_mat = kFALSE;
  _dosys = dosys;
}

inline Int_t RooUnfold::SystematicsIncluded() const {
