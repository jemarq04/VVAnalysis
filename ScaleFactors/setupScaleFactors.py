...
def getComLineArgs():
    ...
    parser.add_argument("-t", "--tightfr_file", type=str, default="")
...
if year == "2016":
    ...
    pileupSF = ROOT.ScaleFactor(
        "pileupSF", "Run2016B-H 35.9/fb Pileup profile over RunIISummer16 MC Scale Factor, x=NTruePU"
    )
    ...
    pileupSF = ROOT.ScaleFactor(
        "pileupSF", "Run2017B-F 41.5/fb Pileup profile over RunIIFall17 MC Scale Factor, x=NTruePU"
    )
    ...
    pileupSF = ROOT.ScaleFactor(
        "pileupSF", "Run2018A-D 59.95/fb Pileup profile over RunIIFall18 MC Scale Factor, x=NTruePU"
    )
...
if year == "2016":
    ...
    muonRun16SF.Set2DHist(
        float2double(mid16File.Get("FINAL")), float2double(mid16File.Get("ERROR")), float2double(mid16File.Get("ERROR"))
    )
    fScales.cd()
    muonRun16SF.Write()
elif year == "2017":
    ...
    muonRun17SF.Set2DHist(
        float2double(midFile.Get("FINAL")), float2double(midFile.Get("ERROR")), float2double(midFile.Get("ERROR"))
    )
    fScales.cd()
    muonRun17SF.Write()
elif year == "2018":
    ...
    muonRun18SF.Set2DHist(
        float2double(mid18File.Get("FINAL")), float2double(mid18File.Get("ERROR")), float2double(mid18File.Get("ERROR"))
    )
    fScales.cd()
    muonRun18SF.Write()
