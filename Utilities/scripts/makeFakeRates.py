...
def getHistNames(channels):
    base_hists = [x + y for x in ["passingLoose", "passingTight"] for y in ["1DEta", "1DPt", "2D"]]
    if len(channels) == 0:
        return base_hists
...
def getDifference(name, dir1, dir2, addRatios=True, composite=True):
    ...
    for histname in getHistNames(channels):
        ...
        if hist1 and hist2:
            ...
            raise RuntimeError("hist %s was not produced for dataset(s) %s and/or %s!" % (histname, dir1, dir2))
...
fileName = "data/fakeRate%s-%s.root" % (today, args["selection"]) if args["output_file"] == "" else args["output_file"]
...
for dataset in ConfigureJobs.getListOfFiles(args["filenames"], path, args["selection"]):
    for chan in channels:
        ...
        if proof:
            proof_path = "_".join([dataset, args["analysis"], args["selection"] + ("#/%s/ntuple" % chan)])
            proof.Process(proof_path, select, "")
            proof_meta_path = "_".join([dataset, args["analysis"], args["selection"] + "#/metaInfo/metaInfo"])
            ...
            try:
                file_path = ConfigureJobs.getInputFilesPath(dataset, path, args["selection"], args["analysis"])
                print(file_path)
                chain.Add(file_path)
...
allewk = makeCompositeHists(
    "AllEWK", ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfEWKFilenames(), path), False
)
