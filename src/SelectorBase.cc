...
void SelectorBase::SetScaleFactors() { std::invalid_argument("No scale factors defined for selector!"); }
...
float SelectorBase::GetPrefiringEfficiencyWeight(std::vector<float>* jetPt, std::vector<float>* jetEta) {
  ...
  if (histData.size() == 4) {
    ...
    if (doSystematics_ && !isNonPrompt_ && std::find(systHists_.begin(), systHists_.end(), name) != systHists_.end()) {
      ...
      for (auto& syst : systematics_) {
        ...
        AddObject<TH1D>(histMap1D_[syst_histName], syst_histName.c_str(), histData[0].c_str(), nbins, xmin, xmax);
