...
RooUnfoldSvd::RooUnfoldSvd(const RooUnfoldSvd& rhs) : RooUnfold(rhs) {
  //! Copy constructor.
  Init();
  CopyData(rhs);
}

RooUnfoldSvd::RooUnfoldSvd(const RooUnfoldResponse* res, const TH1* meas, Int_t kreg, const char* name, const char* title)
...
RooUnfoldSvd::RooUnfoldSvd(const RooUnfoldResponse* res, const TH1* meas, Int_t kreg, Int_t ntoyssvd, const char* name, const char* title)
    : RooUnfold(res, meas, name, title), _kreg(kreg ? kreg : res->GetNbinsTruth() / 2) {
...
RooUnfoldSvd* RooUnfoldSvd::Clone(const char* newname) const {
...
void RooUnfoldSvd::Reset() {
  Destroy();
  Init();
  RooUnfold::Reset();
}

void RooUnfoldSvd::Destroy() {
  delete _svd;
  delete _meas1d;
...
void RooUnfoldSvd::Assign(const RooUnfoldSvd& rhs) {
  RooUnfold::Assign(rhs);
  CopyData(rhs);
}

void RooUnfoldSvd::CopyData(const RooUnfoldSvd& rhs) { _kreg = rhs._kreg; }

TSVDUnfold* RooUnfoldSvd::Impl() { return _svd; }

void RooUnfoldSvd::Unfold() {
  ...
  if (_verbose >= 1)
    cout << "SVD init " << _reshist->GetNbinsX() << " x " << _reshist->GetNbinsY() << " bins, kreg=" << _kreg << endl;
...
void RooUnfoldSvd::GetCov() {
  ...
  if (_dosys)
    adetCov = _svd->GetAdetCovMatrix(_NToys);
  ...
  for (Int_t i = 0; i < _nt; i++) {
    for (Int_t j = 0; j < _nt; j++) {
      Double_t v = 0;
      if (unfoldedCov)
        v = unfoldedCov->GetBinContent(i + 1, j + 1);
      if (adetCov)
        v += adetCov->GetBinContent(i + 1, j + 1);
      _cov(i, j) = v;
    }
  }
...
void RooUnfoldSvd::GetWgt() {
  //! Get weight matrix
  if (_dosys)
    RooUnfold::GetWgt();  // can't add sys errors to weight, so calculate weight from covariance
  if (!_svd)
    return;
  Bool_t oldstat = TH1::AddDirectoryStatus();
  TH1::AddDirectory(kFALSE);
