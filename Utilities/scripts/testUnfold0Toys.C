...
void normalize(TH1 *hist) { hist->Scale(1. / hist->Integral()); }
...
int testUnfold0Toys(TString input = "nominal", int nToys = 100) {
  ...
  for (int t = 0; t < nToys; ++t) {
    if (t % int(nToys / 100) == 0)
      std::cout << "toy number " << t << "/" << nToys << std::endl;
    //============================================
    ...
    for (int i = 1; i < histMunfold->GetNbinsX() + 1; ++i) {
      double pull = (histMunfold->GetBinContent(i) - histMgenToy->GetBinContent(i)) /
                    histMunfold->GetBinError(i);  // sqrt(pow(histMunfold->GetBinError(i), 2) + pow(histMgenToy->GetBinError(i), 2));
