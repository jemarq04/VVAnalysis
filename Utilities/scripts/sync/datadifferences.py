...
for chan in ["eee", "eem", "emm", "mmm"]:
    ...
    for root_file in (
        glob.glob("/data/kelong/DibosonAnalysisData/3LooseLeptons/2017-08-17-data_Single*/*")
    ...
    events = []
    file_name = (
        "/eos/user/k/kelong/WZAnalysisData/SyncWithSvenja/Differences_01Sep2017/all_data_Wselection/%s_%s.txt"
        % (comparison, chan)
    )
    ...
    for i, eventlist in enumerate(chunks(events, 40)):
        ...
        if chan == "eee":
            expr = expr.format(
                ...
                lep2="e2",
                lep3="e3",
                lep1id="IsCBVIDHLTSafe",
                lep2id="IsCBVIDHLTSafe",
                lep3id="IsCBVIDHLTSafe",
            )
        elif chan == "eem":
            expr = expr.format(
                lep1="e1", lep2="e2", lep3="m", lep1id="IsCBVIDHLTSafe", lep2id="IsCBVIDHLTSafe", lep3id="IsWZLoose"
            )
        elif chan == "emm":
            expr = expr.format(
                lep1="m1", lep2="m2", lep3="e", lep1id="IsWZLoose", lep2id="IsWZLoose", lep3id="IsCBVIDHLTSafe"
            )
        elif chan == "mmm":
            expr = expr.format(
                lep1="m1", lep2="m2", lep3="m3", lep1id="IsWZLoose", lep2id="IsWZLoose", lep3id="IsWZLoose"
            )
