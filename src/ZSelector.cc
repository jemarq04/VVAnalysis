...
void ZSelector::Init(TTree* tree) {
  ...
  hists1D_ = {
      "CutFlow", "ZMass", "ZEta",  "yZ",    "ZPt",   "ptl1",  "etal1", "ptl2",  "etal2", "ptj1",
      "ptj2",    "ptj3",  "etaj1", "etaj2", "etaj3", "phij1", "phij2", "phij3", "nJets", "MET",
  };
...
void ZSelector::LoadBranchesNanoAOD(Long64_t entry, std::pair<Systematic, std::string> variation) {
  ...
  if (nTightIdMuon >= 2) {
    channel_ = mm;
    channelName_ = "mm";
    if (!(Muon_mediumId[0] && Muon_pfRelIso04_all[0] < 0.15 && Muon_mediumId[1] && Muon_pfRelIso04_all[1] < 0.15)) {
      for (size_t i = 0; i < nMuon; i++) {
        if (Muon_mediumId[i])
